...
- **`inbox-seed.js`** — przy starcie daemona seeduje joby Team OS (patrz niżej): „Team OS — inbox sync" (script-job co 1 min, `routine=1`, `telegram_notify=1` = alarm o failach) oraz „Team OS — asystent auto-reply" (**seedowany WYŁĄCZONY** — włączenie to decyzja per maszyna). Seed TYLKO gdy inbox skonfigurowany (`INBOX_DB_URL` + `INBOX_USER` przez env-loader); nigdy nie rzuca. **Pułapka**: `loadEnv` skryptów mutuje `process.env` — seed robi snapshot i przywraca go, bo script-joby dziedziczą env daemona i mutacja zamroziłaby konfigurację `INBOX_*` na moment startu.

## Team OS — Skrzynka (`scripts/inbox/`)

Podsystem wymiany wiadomości między członkami zespołu przez współdzielony **Postgres** (tabela `inbox` w `schema.sql`: typy `task`/`query`/`reply`/`close`, statusy `pending`/`delivered`/`done`) z UI w **vaultcie Obsidiana** (plik Skrzynka.md + banner na dashboardzie). Skrypty ESM, odpalane jako script-joby Pulsa (zero CLI Claude poza auto-reply):

- **`inbox-sync.mjs`** — entry point joba: sekwencyjnie push → pull **w jednym procesie** (eliminuje race: user odhacza `[x]` → push UPDATE'uje DB → pull regeneruje plik; osobne joby miały okno, w którym pull nadpisywał akcję usera).
- **`inbox-push.mjs`** — parsuje odhaczone checkboxy z sekcji Otrzymane (markery `%% inbox:items:start/end %%`): task `[x] Zrobione` → reply + `status='done'`; query/reply `[x] Zapoznane` → `done`. Archiwum całych nitek w `Zasoby/inbox-archive/YYYY-MM.md`. Idempotentny.
- **`inbox-pull.mjs`** — renderuje wątki jako callouty Obsidiana (redesign 07.2026: inline HTML spany `os-*` stylowane snippetem `skrzynka.css` w vaultcie; wzorzec `mockup-skrzynka.html`). **Kontrakt push↔pull jest święty**: bloki `> `, marker `%% id/thread %%`, checkbox `- [x] Zrobione|Zapoznane` — zmiany renderera nie mogą go ruszyć (test roundtrip).
- **`auto-reply.mjs`** — MVP autonomii: agent odpowiada na `query` z wiedzy vaulta zanim zrobi to człowiek (spawn `claude -p`, `--allowedTools Read,Glob,Grep`, `cwd`=vault — tylko odczyt, zero akcji zewnętrznych). Atomowy claim przez `payload.auto_reply_attempted` PRZED spawnem; kontrakt `NO_ANSWER` (gdziekolwiek w odpowiedzi) → query zostaje człowiekowi; reply tagowane `🤖 auto-odpowiedź asystenta` + `payload.auto_reply`. Jeden kandydat per run. Historia append-only w `auto-replies-YYYY-MM.md`. Reużywa `resolveClaudeBin`/`buildCleanEnv` z `lib/claude-spawn.js`.
