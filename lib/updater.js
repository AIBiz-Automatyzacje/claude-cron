...
function startUpdate({
  ...
  if (child && typeof child.on === 'function') {
    ...
    // Skrypt WYSZEDŁ z niezerowym kodem — na Macu to normalna ścieżka porażki: `kill`
    // stoi za `&&`, więc przy konflikcie merge albo braku sieci `git pull` pada, a serwer
    // ŻYJE dalej. `'error'` tego nie łapie (spawn się udał), więc bez tego nasłuchu blokada
    // zostawała `true` aż do restartu daemona i każdy kolejny klik dostawał 409
    // „Aktualizacja już trwa", choć nic nie trwało.
    //
    // Kodu 0 świadomie NIE zwalniamy: przy powodzeniu ten proces `exec`-uje nowy serwer,
    // a bieżący i tak zaraz ginie — zwolnienie flagi otworzyłoby okno na drugi instalator
    // w trakcie podmiany plików.
    child.on('exit', code => {
      if (code === 0 || code === null) return;
      updateInProgress = false;
      io.warn(`[updater] Aktualizacja zakończona niepowodzeniem (kod ${code}) — można spróbować ponownie.`);
    });
  }
