...
// sendMessage odrzuca adresatów spoza members — testy wysyłek muszą mieć obie strony.
function seedPair() {
  return { kacper: seedMember('kacper'), kamil: seedMember('kamil') };
}
...
test('send: nieznany adresat → 400 unknown_recipient z listą członków (R1)', () => {
  const token = seedPair().kacper;
  const res = call({ token, action: 'send', method: 'POST', rawBody: JSON.stringify({ to_user: 'kamiI', type: 'task', title: 'T' }) });

  assert.strictEqual(res.status, 400);
  assert.strictEqual(res.json.v, 1);
  assert.strictEqual(res.json.error, 'unknown_recipient');
  assert.deepStrictEqual(res.json.members, ['kacper', 'kamil']); // podpowiedź dla klienta/modelu
  // Literówka nie zostawia sieroty w skrzynce.
  assert.strictEqual(inboxDb.getInboxDb().prepare('SELECT COUNT(*) AS n FROM inbox').get().n, 0);
});

test('send: adresat inną wielkością liter → 200, to_user prostowany do nazwy z members', () => {
  const token = seedMember('kacper');
  seedMember('Cave');
  const res = call({ token, action: 'send', method: 'POST', rawBody: JSON.stringify({ to_user: 'cave', type: 'task', title: 'T' }) });

  assert.strictEqual(res.status, 200);
  assert.strictEqual(res.json.message.to_user, 'Cave');
});
