...
// Równoległe joby (R5): od czasu, gdy runy biegną obok siebie, job sync MUSI deklarować
// wyłączność na współdzielonym artefakcie — przepisuje blok bannera w `Zadania/Dashboard.md`
// w całości, więc równoległy `/daily` (ta sama grupa, ustawiana ręcznie przez usera)
// zgubiłby jedną z wersji bez sygnału.
test('seed: job sync dostaje lock_group="dashboard" (wyłączność na Dashboard.md)', async () => {
  assert.equal(await seedInboxSyncJob(seedOpts()), 'seeded:sync');
  const job = db.getAllJobs().find((j) => j.name === JOB_NAME);
  assert.equal(job.lock_group, 'dashboard');
});

// Ta sama pułapka co przy `enabled`: seed leci przy KAŻDYM boocie, więc „dociąganie"
// istniejącego joba do nowej kolumny (updateJob z lock_group) skasowałoby świadomą decyzję
// usera, który grupę wyczyścił albo zmienił na własną.
test('seed: istniejący job sync → seed NIE dotyka jego lock_group ani enabled (zero UPDATE)', async () => {
  await seedInboxSyncJob(seedOpts());
  const seeded = db.getAllJobs().find((j) => j.name === JOB_NAME);
  db.updateJob(seeded.id, { lock_group: null, enabled: 0 });

  assert.equal(await seedInboxSyncJob(seedOpts()), 'exists:sync');

  const after = db.getAllJobs().filter((j) => j.name === JOB_NAME);
  assert.equal(after.length, 1, 'brak duplikatu obchodzącego decyzję usera');
  assert.equal(after[0].lock_group, null, 'ręcznie wyczyszczona grupa wyłączności przeżywa restart');
  assert.equal(after[0].enabled, 0, 'ręczne wyłączenie przeżywa restart');
});
...
// ZMIANA KONTRAKTU 07.08 (decyzja Kacpra po incydencie 06.08): stary job poprzedniej roli
// NIE zostaje już włączony — dwie maszyny renderujące Skrzynkę pod Obsidian Sync uszkodziły
// plik na żywo. Seed dalej niczego nie KASUJE (job zostaje w panelu jako wyłączony, z historią),
// ale egzekwuje wyłączność: job przeciwnej roli jest wyłączany przy każdym boocie.
test('seed: ZMIANA roli client → agent WYŁĄCZA stary sync (egzekwowanie wyłączności ról)', async () => {
  db.setState(ROLE_STATE_KEY, 'client');
  assert.equal(await seedInboxSyncJob(seedOpts()), 'seeded:sync');
  ...
  assert.equal(syncAfter.enabled, 0, 'sync na maszynie agent MUSI być wyłączony (incydent 06.08)');
...
// === Egzekwowanie wyłączności ról (incydent 06.08: sync włączony na hubie-agencie) ===

test('rola agent + WŁĄCZONY sync → sync zostaje wyłączony przy boocie', async () => {
  db.setState(ROLE_STATE_KEY, 'agent');
  db.createJob({ name: JOB_NAME, job_type: 'script', command: '/x/inbox-sync.mjs', cron_expr: '*/1 * * * *', enabled: 1 });

  await seedInboxSyncJob(seedOpts());

  const sync = db.getAllJobs().find((j) => j.name === JOB_NAME);
  assert.equal(sync.enabled, 0); // niezmiennik: dwie maszyny nie renderują Skrzynki naraz
  const assistant = db.getAllJobs().find((j) => j.name === ASSISTANT_JOB_NAME);
  assert.ok(assistant, 'auto-reply ma powstać mimo egzekwowania');
  assert.equal(assistant.enabled, 1);
});

test('rola client + WŁĄCZONY auto-reply → auto-reply wyłączony, sync powstaje', async () => {
  db.setState(ROLE_STATE_KEY, 'client');
  db.createJob({ name: ASSISTANT_JOB_NAME, job_type: 'script', command: '/x/auto-reply.mjs', cron_expr: '*/1 * * * *', enabled: 1 });

  await seedInboxSyncJob(seedOpts());

  assert.equal(db.getAllJobs().find((j) => j.name === ASSISTANT_JOB_NAME).enabled, 0);
  assert.ok(db.getAllJobs().find((j) => j.name === JOB_NAME));
});

test('BRAK flagi roli → egzekwowanie nie rusza niczego (instalacje ręczne sprzed ról)', async () => {
  db.createJob({ name: ASSISTANT_JOB_NAME, job_type: 'script', command: '/x/auto-reply.mjs', cron_expr: '*/1 * * * *', enabled: 1 });

  await seedInboxSyncJob(seedOpts());

  // brak flagi = nie zgadujemy roli — ręcznie włączony auto-reply zostaje
  assert.equal(db.getAllJobs().find((j) => j.name === ASSISTANT_JOB_NAME).enabled, 1);
});

test('rola agent + sync JUŻ wyłączony → zero UPDATE (nie budzimy joba, nie spamujemy loga)', async () => {
  db.setState(ROLE_STATE_KEY, 'agent');
  db.createJob({ name: JOB_NAME, job_type: 'script', command: '/x/inbox-sync.mjs', cron_expr: '*/1 * * * *', enabled: 0 });
  const before = db.getAllJobs().find((j) => j.name === JOB_NAME);

  await seedInboxSyncJob(seedOpts());

  const after = db.getAllJobs().find((j) => j.name === JOB_NAME);
