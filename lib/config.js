...
// Override ścieżki bazy przez env — dla testów HTTP na ŻYWYM procesie serwera
// (lib/ask.http.test.js): in-process db.setDbPath nie sięga spawnowanego dziecka,
// a test tworzący joby/runy NIE MOŻE pisać do realnej bazy usera.
const DB_PATH = process.env.CLAUDE_CRON_DB_PATH || path.join(DATA_DIR, 'claude-cron.db');
// Osobny plik bazy Team OS (hub skrzynki) — dane zespołowe nie mieszają się z jobami,
// backup/kasowanie niezależny od claude-cron.db. Override przez env analogicznie do
// CLAUDE_CRON_DB_PATH — testy HTTP na żywym procesie serwera nie mogą pisać do realnej bazy.
const INBOX_DB_PATH = process.env.CLAUDE_CRON_INBOX_DB_PATH || path.join(DATA_DIR, 'inbox.db');
...
// Timeouty z override przez env (plan: korekta limitu Shortcuts/Watch bez zmiany kodu);
// Number(undefined/'')/nie-liczba → NaN/0 → fallback na default.
const ASK_TIMEOUT_MS = Number(process.env.ASK_TIMEOUT_MS) || 55_000; // okno synchronicznej odpowiedzi — poniżej limitu 60s klienta głosowego
const ASK_MAX_MS = Number(process.env.ASK_MAX_MS) || 10 * 60 * 1000; // twardy limit życia odczepionego procesu tła
...
// Claude CLI — override przez env dla testów HTTP na żywym procesie serwera
// (atrapa CLI zamiast prawdziwej binarki; in-process setClaudeBin nie sięga
// spawnowanego dziecka, a env czytany jest raz przy starcie procesu).
const CLAUDE_BIN = process.env.CLAUDE_CRON_CLAUDE_BIN || 'claude';
