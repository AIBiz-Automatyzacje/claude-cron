...
const { spawnClaude, resolveClaudeBin, buildCleanEnv, setClaudeBin } = require('./claude-spawn');
...
// === resolveClaudeBin — gałąź Windows przez wstrzyknięte deps (martwa na Macu) ===

test('resolveClaudeBin: Windows, `where claude` znajduje binarkę → pierwsza linia wyniku', () => {
  // Arrange — atrapa exec zamiast realnego execSync (zewnętrzny serwis: cmd.exe)
  const exec = () => 'C:\\Users\\kacper\\AppData\\Local\\bin\\claude.exe\r\nC:\\inna\\claude.exe\n';

  // Act
  const bin = resolveClaudeBin({ isWin: true, exec });

  // Assert
  assert.equal(bin, 'C:\\Users\\kacper\\AppData\\Local\\bin\\claude.exe');
});

test('resolveClaudeBin: Windows, `where claude` pada → czytelny błąd, ZERO fallbacku shell:true', () => {
  // Arrange — `where` bez trafienia kończy się kodem 1 → execSync rzuca
  const exec = () => { throw new Error('Command failed: where claude'); };

  // Act / Assert — kontrakt po review P2: fail-fast z instrukcją, nie shell:true
  assert.throws(
    () => resolveClaudeBin({ isWin: true, exec }),
    /Nie znaleziono binarki `claude` w PATH/
  );
});
