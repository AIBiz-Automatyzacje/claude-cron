...
// Twardy cap nagłówka ✅ — bez niego nazwa joba ~4090+ znaków (API nie waliduje długości
// name) dawała ujemny limit chunka i nieskończoną pętlę w smartSplit (DoS na event loopie).
const HEADER_MAX_LEN = 1000;
...
function buildMessages(jobName, stdout) {
  const header = `✅ ${jobName}`.slice(0, HEADER_MAX_LEN);
  const chunks = smartSplit(extractResult(stdout), TELEGRAM_MAX_LEN - header.length - 1);
  // Wynik z samych białych znaków → zero chunków; sam nagłówek zamiast pustego texta (400).
...
// Seam plain-text dla ask (asystent głosowy): surowy tekst bez nagłówka buildMessages
// i BEZ extractResult — ask używa `--output-format text`, nie stream-json. Chunking przez
// smartSplit (≤4096, plain text bez parse_mode), konfiguracja w czasie wysyłki jak wyżej.
async function sendPlain(text) {
  const { telegramBotToken, telegramChatId } = resolveNotifyConfig(db.getState, process.env);
  if (!telegramBotToken || !telegramChatId) return;

  for (const chunk of smartSplit(text, TELEGRAM_MAX_LEN)) {
    await postSendMessage(telegramBotToken, telegramChatId, chunk);
  }
}

module.exports = { sendNotification, sendFailureNotification, sendPlain, buildMessages, buildFailureMessage };
