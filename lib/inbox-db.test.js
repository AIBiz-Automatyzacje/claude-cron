...
test('migrate: kolizja "Cave" + "cave" → czytelny błąd z obiema nazwami, baza nietknięta', () => {
  const db = legacyDb(['Cave', 'cave']);

  assert.throws(
    () => inboxDb.migrate(db),
    (err) => err instanceof InboxDbError && err.message.includes('"Cave" + "cave"')
  );

  // Dane bez zmian, schemat wciąż legacy (fail-fast, nie ciche scalenie).
