...
// Self-executing guard. Sprawdza wersję Node; jeśli poza zakresem → komunikat na
// stderr + exit(1). Wersja oraz efekty (exit/stderr) wstrzykiwane argumentami
// (DI), by droga fail-fast była testowalna bez ubijania procesu testów; domyślne
// wartości to realny runtime + realne process.exit/stderr.
function enforceNodeVersion(
  ...
  if (!isNodeSupported(version)) {
    onFail(buildUnsupportedMessage(version));
  }
}
