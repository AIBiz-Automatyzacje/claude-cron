...
// === Szew ask+reaper: ❌ „przerwane przez restart" (Unit 6, R7) ===
// Test integracyjny obu stron na :memory: (lekcja stale-obiekt-vs-DB: testy czystych
// funkcji reapera i ask osobno przechodzą przy złamanym zachowaniu systemowym).
// Sekwencja Act = dokładnie ta ze startu serwera: reapOrphanedRuns → notifyInterruptedAskRuns.

test('szew ask+reaper: run teczki running sprzed restartu → killed + dokładnie jedno ❌ o restarcie', (t) => {
  // Arrange — teczka z telegramem, osierocony run ask (symulacja odczepionego zadania
  // przerwanego restartem serwera)
  const channels = mockChannels(t);
  const job = getOrCreateAskJob();
  db.updateJob(job.id, { telegram_notify: 1 });
  const run = db.createRun({ job_id: job.id, trigger_type: 'ask', webhook_payload: 'pytanie sprzed restartu' });
  db.updateRun(run.id, { status: 'running', started_at: new Date().toISOString() });

  // Act — logika startowa serwera
  const reaped = db.reapOrphanedRuns();
  notifyInterruptedAskRuns(reaped);

  // Assert — run domknięty jako killed, jedno ❌ z tekstem o restarcie na właściwym kanale
  assert.equal(db.getRunWithPayload(run.id).status, 'killed');
  assert.equal(channels.telegram.mock.callCount(), 1, 'dokładnie jedno ❌');
  const message = channels.telegram.mock.calls[0].arguments[0];
  assert.ok(message.startsWith('❌'), 'powiadomienie o przerwaniu zaczyna się od ❌');
  assert.ok(message.includes('przerwane przez restart serwera'), 'tekst wskazuje restart jako przyczynę');
  assert.ok(message.includes('poproś jeszcze raz'), 'tekst prosi o ponowienie pytania');
  assert.equal(channels.discord.mock.callCount(), 0, 'flaga discord_notify=0 → kanał milczy');
});

test('szew ask+reaper: osierocony run ZWYKŁEGO joba → reap bez żadnego powiadomienia', (t) => {
  // Arrange — zwykły job z WŁĄCZONYMI kanałami (cisza ma świadczyć o filtrze po teczce,
  // nie o wyłączonych flagach); teczka ask NIE istnieje
  const channels = mockChannels(t);
  const job = db.createJob({
    name: 'Zwykły cron', skill_name: 's', cron_expr: '*/5 * * * *',
    discord_notify: 1, telegram_notify: 1,
  });
  ...
  // Assert — kontrakt „killed milczy" zachowany, a start serwera nie seeduje teczki
  assert.equal(db.getRunWithPayload(run.id).status, 'killed');
  assert.equal(channels.telegram.mock.callCount(), 0);
  assert.equal(channels.discord.mock.callCount(), 0);
  assert.equal(db.getAllJobs().length, 1, 'lookup teczki bez tworzenia — czysta instalacja bez seedu');
});

test('szew ask+reaper: brak osieroconych runów → notifyInterruptedAskRuns to no-op', (t) => {
  // Arrange — teczka z kanałem istnieje, ale nic nie było przerwane
  const channels = mockChannels(t);
  const job = getOrCreateAskJob();
  db.updateJob(job.id, { telegram_notify: 1 });
