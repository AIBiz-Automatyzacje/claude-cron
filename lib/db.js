...
function createJob({ name, skill_name = '', cron_expr = '', arguments: args = '', enabled = 1, run_on_wake = 0, timeout_ms = 600000, max_retries = 1, discord_notify = 0, job_type = 'claude', command = null }) {
  const stmt = getDb().prepare(`
    INSERT INTO jobs (name, skill_name, cron_expr, arguments, enabled, run_on_wake, timeout_ms, max_retries, discord_notify, job_type, command)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const result = stmt.run(name, skill_name, cron_expr, args, enabled ? 1 : 0, run_on_wake ? 1 : 0, timeout_ms, max_retries, discord_notify ? 1 : 0, job_type, command);
  return getJob(result.lastInsertRowid);
}

function updateJob(id, fields) {
  const allowed = ['name', 'skill_name', 'cron_expr', 'arguments', 'enabled', 'run_on_wake', 'timeout_ms', 'max_retries', 'discord_notify', 'job_type', 'command'];
  const updates = [];
  const values = [];
