...
function getRuns({ limit = 50, offset = 0, job_id, hideRoutine = false, fields } = {}) {
  const cols = fields === 'meta' ? RUN_META_COLUMNS : 'r.*';
  if (job_id) {
    return getDb().prepare(`SELECT ${cols} FROM runs r WHERE r.job_id = ? ORDER BY r.id DESC LIMIT ? OFFSET ?`).all(job_id, limit, offset);
  }
  if (hideRoutine) {
  ...
  return getDb().prepare(`SELECT ${cols} FROM runs r ORDER BY r.id DESC LIMIT ? OFFSET ?`).all(limit, offset);
}
