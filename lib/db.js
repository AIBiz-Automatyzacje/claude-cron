...
function createJob({ name, skill_name = '', cron_expr = '', arguments: args = '', enabled = 1, run_on_wake = 0, timeout_ms = 600000, idle_timeout_ms = 300000, max_retries = 1, discord_notify = 0, job_type = 'claude', command = null }) {
  const stmt = getDb().prepare(`
    INSERT INTO jobs (name, skill_name, cron_expr, arguments, enabled, run_on_wake, timeout_ms, idle_timeout_ms, max_retries, discord_notify, job_type, command)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const result = stmt.run(name, skill_name, cron_expr, args, enabled ? 1 : 0, run_on_wake ? 1 : 0, timeout_ms, idle_timeout_ms, max_retries, discord_notify ? 1 : 0, job_type, command);
  return getJob(result.lastInsertRowid);
}

function updateJob(id, fields) {
  const allowed = ['name', 'skill_name', 'cron_expr', 'arguments', 'enabled', 'run_on_wake', 'timeout_ms', 'idle_timeout_ms', 'max_retries', 'discord_notify', 'job_type', 'command'];
  const updates = [];
  const values = [];
...
function getRuns({ limit = 50, offset = 0, job_id, hideRoutine = false } = {}) {
  ...
  if (hideRoutine) {
    // Ukryj successful runy script-jobów (np. inbox-pull co minutę). Pokaż fails + wszystkie claude runs.
    return getDb().prepare(`
      SELECT r.* FROM runs r
      LEFT JOIN jobs j ON j.id = r.job_id
      WHERE NOT (COALESCE(j.job_type,'claude') = 'script' AND r.status = 'success')
...
// Retention — usuń successful runy script-jobów starsze niż N godzin.
// Failed/timeout/killed zostają forever dla debug, claude jobs też zostają.
function deleteOldRoutineRuns(olderThanHours = 24) {
  ...
  const result = getDb().prepare(`
    ...
    WHERE status = 'success'
      AND finished_at < ?
      AND job_id IN (SELECT id FROM jobs WHERE job_type = 'script')
  `).run(cutoff);
