...
function executeAsk(question, {
  ...
  return new Promise((resolve) => {
    ...
    let detached = false;
    let timedOut = false;
    let settled = false; // 'close' i 'error' mogą przyjść OBA (pad spawnu) — wspólna ścieżka domyka raz
    ...
    const maxTimerId = setTimeout(() => {
      ...
      finalizeAskRun({
        ...
        stdout,
        stderr,
        errorMsg: `Przekroczony limit ${askMaxMs}ms (ASK_MAX_MS)`,
    ...
    proc.on('close', (code) => {
      ...
      const errorMsg = timedOut ? `Przekroczony limit ${askMaxMs}ms (ASK_MAX_MS)` : '';
      settle({ status, code, errorMsg });
    });
