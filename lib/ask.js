...
function killProcessTree(proc) {
  ...
  // Bez try/catch — kill na zakończonym procesie zwraca `false` bez wyjątku, a EPERM
  // przychodzi zdarzeniem 'error' (pełny kontrakt Node opisany przy killProcessTree
  // w lib/executor.js).
  proc.kill('SIGTERM');
  setTimeout(() => proc.kill('SIGKILL'), 5000).unref();
}

// Jedyna ścieżka wysyłki plain-text na kanały wg flag teczki — wspólny szew dla ✅/❌
// wyniku odczepionego zadania ORAZ ❌ przerwania restartem (Unit 6). Fire-and-forget:
// pad kanału tylko do logu, nigdy nie propaguje (nie może blokować startu serwera).
// Oba kanały wyłączone = jedyny dopuszczalny „cichy" przypadek — jawnie zalogowany.
function sendAskNotification(job, text) {
...
// retry/„killed milczy" — nietknięty).
function notifyAskOutcome(job, status, { stdout, stderr, errorMsg }) {
  ...
  sendAskNotification(job, text);
}

// ❌ dla runów teczki przerwanych restartem serwera (Unit 6, R7 „nigdy cisza").
// Wołane RAZ ze startu serwera z listą zebranych przez reaper. Lookup teczki po `name`
// BEZ tworzenia — reaper nie może seedować teczki na czystej instalacji (teczka powstaje
// wyłącznie przy realnym użyciu /ask). Runy zwykłych jobów ignorowane (kontrakt
// „killed milczy"). Jedno ❌ per przerwany run — każde odczepione pytanie ma swój wynik.
function notifyInterruptedAskRuns(reapedRuns) {
  if (!reapedRuns || reapedRuns.length === 0) return;
  const askJob = db.getAllJobs().find((job) => job.name === ASK_JOB_NAME);
  if (!askJob) return;
  const interrupted = reapedRuns.filter((run) => run.job_id === askJob.id);
  for (const run of interrupted) {
    console.log(`[ask] run #${run.id}: przerwany przez restart serwera — wysyłam ❌`);
    sendAskNotification(askJob, `❌ ${askJob.name} — zadanie przerwane przez restart serwera, poproś jeszcze raz`);
  }
}
