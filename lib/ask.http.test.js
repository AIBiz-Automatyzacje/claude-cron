...
// POST surowym http.request z body rozciętym na dwa write'y z pauzą — wymusza
// osobne zdarzenia 'data' po stronie serwera (fetch skleiłby body w jeden write,
// więc nie odtworzy granicy chunków, które za proxy Funnel są poza kontrolą).
function postSplitBody(port, urlPath, headers, buf, splitAt) {
  return new Promise((resolve, reject) => {
    const req = http.request({ port, path: urlPath, method: 'POST', headers }, (res) => {
      let data = '';
...
test('znak UTF-8 rozcięty między chunki → pytanie w payloadzie runu bez U+FFFD', { skip: SKIP_WIN }, async () => {
  // Arrange — rozcięcie w ŚRODKU dwubajtowego 'ę' (0xC4 0x99): pierwszy chunk kończy
  // się na 0xC4, drugi zaczyna od 0x99. Split liczony po bajcie 0xC4, NIE po długości
  // stringa — split na końcu bufora rozcinałby ASCII i test przechodziłby vacuously.
  const question = 'pytanie o pogodę rozcięte';
  const buf = Buffer.from(question, 'utf8');
  const splitAt = buf.indexOf(0xC4) + 1;
  assert.ok(splitAt > 0 && splitAt < buf.length, 'split wypada w środku znaku wielobajtowego');

  // Act
  const res = await postSplitBody(
    PORT_ENABLED, `/ask/${TEST_TOKEN}`, { 'X-Secret': TEST_SECRET }, buf, splitAt,
  );

  // Assert — odpowiedź normalna, a pytanie dotarło w całości (bez U+FFFD)
  assert.equal(res.status, 200);
  assert.equal(res.text, FAKE_ANSWER);
  const runs = await (await fetch(urlEnabled('/api/runs'))).json();
  const run = runs.find((r) => r.webhook_payload === question);
  assert.ok(run, 'run z pełnym pytaniem istnieje — diakrytyk przeżył granicę chunków');
  assert.ok(
    !runs.some((r) => typeof r.webhook_payload === 'string' && r.webhook_payload.includes('�')),
    'żaden payload nie zawiera U+FFFD',
  );
});

test('body ponad limit → 413 bez tworzenia runu, serwer dalej obsługuje pytania', { skip: SKIP_WIN }, async () => {
  // Arrange
  const runsBefore = (await (await fetch(urlEnabled('/api/runs'))).json()).length;
  const hugeBody = 'a'.repeat(100 * 1024); // 100 KB > cap 64 KB
  ...
  // Assert — odmowa bez akumulacji i bez runu
  assert.equal(res.status, 413);
  const runsAfter = (await (await fetch(urlEnabled('/api/runs'))).json()).length;
  assert.equal(runsAfter, runsBefore, 'żaden run nie powstał z odrzuconego body');

  // Serwer przeżył i nie wyciekła żadna rezerwacja (odmowa PRZED admitRequest)
  const alive = await fetch(urlEnabled(`/ask/${TEST_TOKEN}`), {
    method: 'POST',
    headers: { 'X-Secret': TEST_SECRET },
    body: 'czy dalej działasz',
  });
  assert.equal(alive.status, 200);
  assert.equal(await alive.text(), FAKE_ANSWER);
});
