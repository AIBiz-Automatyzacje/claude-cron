...
// Dziedzina roli eksportowana razem z walidatorem, bo PISARZ tej wartości mieszka gdzie
// indziej (instalatory: scripts/inbox/onboard.mjs, setup.mjs), a porównanie niżej jest
// strict — literał przepisany na ślepo ('Agent', 'agent\n') cicho degradowałby maszynę
// do klienta. Jedno źródło słownika zamiast trzech kopii stringów.
function isValidRole(value) {
  return value === ROLE_AGENT || value === ROLE_CLIENT;
}
...
function inboxSyncJobDef(repoRoot) {
  return {
    ...
    // Wyłączność deklaratywna (R5): pull przepisuje blok bannera w `Zadania/Dashboard.md`
    // w całości, więc równoległy run innego joba dotykającego tego pliku (np. `/daily`)
    // zgubiłby jedną z wersji bez żadnego sygnału. Grupa = WSPÓŁDZIELONY ARTEFAKT, nie
    // „cały vault" — grupa `vault` odtworzyłaby stary globalny slot.
    lock_group: 'dashboard',
  };
}
...
// Wyłącza WŁĄCZONY job przeciwnej roli (agent↔sync, client↔auto-reply). Wołane tylko
// przy jawnej roli. Wyłączenie zamiast kasowania: job z historią runów zostaje w panelu
// jako wyłączony — widać, że COŚ tu było i zostało zatrzymane, zamiast znikać bez śladu.
function enforceRoleExclusivity(role) {
  const forbiddenName = role === ROLE_AGENT ? JOB_NAME : ASSISTANT_JOB_NAME;
  const job = db.getAllJobs().find((j) => j.name === forbiddenName && j.enabled);
  if (!job) return;
  db.updateJob(job.id, { enabled: 0 });
  console.error(
    `[seed] ⚠️ Rola maszyny to "${role}", a job "${forbiddenName}" był WŁĄCZONY — wyłączam. ` +
      'Sync biega wyłącznie na maszynie człowieka (client), auto-reply wyłącznie na VPS (agent); ' +
      'dwie maszyny renderujące Skrzynkę pod Obsidian Sync uszkadzają plik (incydent 06.08).'
  );
}
...
//
// onJobCreated — hak wołany WYŁĄCZNIE po realnym utworzeniu joba. Seed jest async
// (`await loadEnvFn`), a `scheduler.start()` w server.js leci synchronicznie zaraz po nim,
// więc czyta listę jobów, w której świeżego joba jeszcze NIE MA. Bez tego haka job zostawał
// bez harmonogramu do następnego restartu daemona — pierwsza instalacja miała martwą
// Skrzynkę (rola client) albo milczącego asystenta (rola agent). Seed NIE zna schedulera
// (zero couplingu) — dostaje go przez DI od wołającego.
async function seedInboxSyncJob({
  loadEnvFn = defaultLoadEnv,
  ...
  const role = db.getState(ROLE_STATE_KEY);
  const isAgent = role === ROLE_AGENT;
  // Egzekwowanie wyłączności ról PO FAKCIE — seed tworzy właściwy job, ale ręczne włączenie
  // niewłaściwego (panel/API) przechodziło bez słowa. Incydent 06.08: hub (rola agent) miał
  // włączony sync → dwie maszyny renderowały ten sam Skrzynka.md co minutę i Obsidian Sync
  // scalił plik tekstowo, USZKADZAJĄC go. Wyłączamy więc job PRZECIWNEJ roli przy każdym
  // boocie — wyłącznie przy roli JAWNEJ (brak flagi = instalacja ręczna sprzed ról, nie
  // zgadujemy). To NIE łamie zasady „nigdy updateJob": tamta chroni ręczne wyłączenia joba
  // WŁASNEJ roli; tu egzekwujemy niezmiennik bezpieczeństwa danych (topologia sync/agent).
  if (isValidRole(role)) enforceRoleExclusivity(role);
  ...
  // czyli dokładnie zachowanie sprzed tej poprawki).
