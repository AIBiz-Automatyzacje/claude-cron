...
// === Detekcja drugiego schedulera na tej samej bazie (incydent 07.08: zabłąkany server.js) ===

test('isForeignSchedulerAlive: cudzy odcisk = intruz, własny/pusty = spokój', () => {
  const mine = '123:1700000000000';
  // Obcy odcisk w state znaczy: ktoś nadpisał wartość PO naszym ostatnim zapisie — żyje.
  assert.equal(scheduler.isForeignSchedulerAlive({ stored: '999:1700000009999', mine }), true);
  // Nasz własny zapis — nikt nie ruszał.
  assert.equal(scheduler.isForeignSchedulerAlive({ stored: mine, mine }), false);
  // Brak wartości (świeża baza / stary schemat) — nie ma dowodu na intruza.
  assert.equal(scheduler.isForeignSchedulerAlive({ stored: undefined, mine }), false);
  assert.equal(scheduler.isForeignSchedulerAlive({ stored: null, mine }), false);
  assert.equal(scheduler.isForeignSchedulerAlive({ stored: '', mine }), false);
});
