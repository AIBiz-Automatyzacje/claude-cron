const { test, before, after, beforeEach } = require('node:test');
const assert = require('node:assert/strict');

const db = require('./db');

// Izolacja testów w warstwie testu (DI), bez dotykania config.js:
// baza in-memory — szybka, izolowana, nie śmieci plikiem (źródło §292).
before(() => {
  db.setDbPath(':memory:');
  db.getDb();
});

after(() => {
  db.close();
});

beforeEach(() => {
  const conn = db.getDb();
  conn.exec('DELETE FROM runs; DELETE FROM jobs;');
});

function seedRuns(jobId, count) {
  const conn = db.getDb();
  const stmt = conn.prepare(
    "INSERT INTO runs (job_id, status, trigger_type) VALUES (?, 'success', 'scheduled')"
  );
  for (let i = 0; i < count; i += 1) {
    stmt.run(jobId);
  }
}

// Wstawia run ze started_at jako UTC ISO (tak jak produkcja: new Date().toISOString()).
function seedRunAt(jobId, startedAtIso, status) {
  const conn = db.getDb();
  conn
    .prepare(
      "INSERT INTO runs (job_id, status, trigger_type, started_at) VALUES (?, ?, 'scheduled', ?)"
    )
    .run(jobId, status, startedAtIso);
}

// Zwraca UTC ISO odpowiadający lokalnej godzinie `hour:minute` dnia (dziś + dayOffset).
// Konstruujemy moment przez lokalne pola, więc toISOString() zwraca poprawny UTC
// niezależnie od strefy — dokładnie tak jak produkcyjny new Date().toISOString().
function localMomentIso(dayOffset, hour = 12, minute = 0) {
  const d = new Date();
  d.setDate(d.getDate() + dayOffset);
  d.setHours(hour, minute, 0, 0);
  return d.toISOString();
}

// Wstawia pojedynczy run o zadanym statusie i finished_at (ISO lub null) — zwraca id.
function seedRunWith(jobId, status, finishedAtIso = null) {
  const conn = db.getDb();
  const result = conn
    .prepare(
      "INSERT INTO runs (job_id, status, trigger_type, finished_at) VALUES (?, ?, 'scheduled', ?)"
    )
    .run(jobId, status, finishedAtIso);
  return result.lastInsertRowid;
}

// ISO odpowiadający chwili sprzed N godzin (UTC, jak produkcja).
function hoursAgoIso(hours) {
  return new Date(Date.now() - hours * 3600000).toISOString();
}

test('getTodayRunStats jest wyeksportowany', () => {
  assert.equal(typeof db.getTodayRunStats, 'function');
});

test('liczy tylko dzisiejsze runy i rozdziela success vs failed', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunAt(job.id, localMomentIso(0, 9, 0), 'success');
  seedRunAt(job.id, localMomentIso(0, 10, 0), 'success');
  seedRunAt(job.id, localMomentIso(0, 11, 0), 'failed');
  // wczoraj — nie powinno być liczone
  seedRunAt(job.id, localMomentIso(-1, 23, 30), 'success');
  seedRunAt(job.id, localMomentIso(-1, 23, 45), 'failed');

  // Act
  const stats = db.getTodayRunStats();

  // Assert
  assert.equal(stats.success, 2);
  assert.equal(stats.failed, 1);
});

test('timeout i killed wliczane do failed', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunAt(job.id, localMomentIso(0, 8, 0), 'timeout');
  seedRunAt(job.id, localMomentIso(0, 8, 30), 'killed');
  seedRunAt(job.id, localMomentIso(0, 9, 0), 'success');

  // Act
  const stats = db.getTodayRunStats();

  // Assert
  assert.equal(stats.success, 1);
  assert.equal(stats.failed, 2);
});

test('run tuż po północy lokalnej liczony jako dziś (regresja UTC)', () => {
  // Arrange — run o 00:30 lokalnego czasu dzisiaj.
  // Dla stref dodatnich (np. PL UTC+1/+2) ten moment w UTC to jeszcze WCZORAJ.
  // date(started_at,'localtime') = date('now','localtime') musi zaliczyć go do dziś.
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunAt(job.id, localMomentIso(0, 0, 30), 'success');

  // Act
  const stats = db.getTodayRunStats();

  // Assert
  assert.equal(stats.success, 1);
  assert.equal(stats.failed, 0);
});

test('brak runów dziś → {success:0, failed:0} (nie null)', () => {
  // Arrange — tylko wczorajszy run
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunAt(job.id, localMomentIso(-1, 12, 0), 'success');

  // Act
  const stats = db.getTodayRunStats();

  // Assert
  assert.equal(stats.success, 0);
  assert.equal(stats.failed, 0);
});

test('pomija joby routine=1 — statbar bez szumu (np. inbox sync co minutę)', () => {
  // Arrange — nierutynowy job + rutynowy job, oba z dzisiejszymi runami
  const normal = db.createJob({ name: 'A', skill_name: 's', cron_expr: '0 9 * * *' });
  const routine = db.createJob({ name: 'inbox', skill_name: 's', cron_expr: '*/1 * * * *', routine: 1 });
  seedRunAt(normal.id, localMomentIso(0, 9, 0), 'success');
  seedRunAt(routine.id, localMomentIso(0, 9, 1), 'success');
  seedRunAt(routine.id, localMomentIso(0, 9, 2), 'failed');

  // Act
  const stats = db.getTodayRunStats();

  // Assert — liczony tylko nierutynowy job
  assert.equal(stats.success, 1);
  assert.equal(stats.failed, 0);
});

test('getRecentRunsPerJob jest wyeksportowany', () => {
  assert.equal(typeof db.getRecentRunsPerJob, 'function');
});

test('zwraca dokładnie N runów dla joba o dużej kadencji, mniej gdy mniej runów', () => {
  // Arrange — job A (częsty, 20 runów), job B (rzadki, 3 runy)
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '0 0 * * *' });
  seedRuns(jobA.id, 20);
  seedRuns(jobB.id, 3);

  // Act
  const rows = db.getRecentRunsPerJob(7);

  // Assert — A dostaje 7 (cap), B dostaje 3 (wszystkie, bo mniej niż N)
  const aRuns = rows.filter(r => r.job_id === jobA.id);
  const bRuns = rows.filter(r => r.job_id === jobB.id);
  assert.equal(aRuns.length, 7);
  assert.equal(bRuns.length, 3);
});

test('wynik posortowany DESC po id w obrębie joba', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRuns(job.id, 10);

  // Act
  const rows = db.getRecentRunsPerJob(7).filter(r => r.job_id === job.id);

  // Assert — id malejące
  for (let i = 1; i < rows.length; i += 1) {
    assert.ok(rows[i - 1].id > rows[i].id, 'runy joba muszą być DESC po id');
  }
});

test('per_job=0 → fallback default (nie pusta tablica gdy są runy)', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRuns(job.id, 10);

  // Act — 0 jest nieprawidłowe, helper musi użyć defaultu
  const rows = db.getRecentRunsPerJob(0);

  // Assert — default to 7
  const jobRuns = rows.filter(r => r.job_id === job.id);
  assert.equal(jobRuns.length, 7);
});

test('brak argumentu → fallback default 7', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRuns(job.id, 10);

  // Act
  const rows = db.getRecentRunsPerJob();

  // Assert
  assert.equal(rows.filter(r => r.job_id === job.id).length, 7);
});

test('per_job ponad cap → przycięty do cap (50)', () => {
  // Arrange — 60 runów, żądanie 999
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRuns(job.id, 60);

  // Act
  const rows = db.getRecentRunsPerJob(999);

  // Assert — przycięte do 50
  assert.equal(rows.filter(r => r.job_id === job.id).length, 50);
});

// === getRuns({ hideRoutine }) ===

test('hideRoutine ukrywa SUCCESS rutynowego joba, ale jego FAIL zostaje widoczny', () => {
  // Arrange — rutynowy job (routine=1) z udanym i nieudanym runem
  const routine = db.createJob({ name: 'inbox', skill_name: 's', cron_expr: '*/1 * * * *', routine: 1 });
  const okId = seedRunWith(routine.id, 'success');
  const failId = seedRunWith(routine.id, 'failed');

  // Act
  const rows = db.getRuns({ hideRoutine: true });

  // Assert — success ukryty, fail widoczny
  const ids = rows.map(r => r.id);
  assert.ok(!ids.includes(okId), 'udany run rutynowego joba musi być ukryty');
  assert.ok(ids.includes(failId), 'nieudany run rutynowego joba musi być widoczny');
});

test('hideRoutine pokazuje WSZYSTKIE runy nierutynowego joba (także success)', () => {
  // Arrange — nierutynowy job (routine=0 default)
  const normal = db.createJob({ name: 'report', skill_name: 's', cron_expr: '0 9 * * *' });
  const okId = seedRunWith(normal.id, 'success');
  const failId = seedRunWith(normal.id, 'failed');

  // Act
  const rows = db.getRuns({ hideRoutine: true });

  // Assert — oba widoczne
  const ids = rows.map(r => r.id);
  assert.ok(ids.includes(okId), 'success nierutynowego joba zawsze widoczny');
  assert.ok(ids.includes(failId), 'fail nierutynowego joba zawsze widoczny');
});

// === getRuns({ job_id }) ===

test('job_id filtruje runy tylko danego joba, DESC po id, respektuje limit', () => {
  // Arrange — dwa joby, każdy z runami
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRuns(jobA.id, 5);
  seedRuns(jobB.id, 3);

  // Act — tylko jobA, limit 3
  const rows = db.getRuns({ job_id: jobA.id, limit: 3 });

  // Assert — wszystkie należą do jobA
  assert.equal(rows.length, 3);
  assert.ok(rows.every(r => r.job_id === jobA.id), 'tylko runy jobA');
  // DESC po id
  for (let i = 1; i < rows.length; i += 1) {
    assert.ok(rows[i - 1].id > rows[i].id, 'runy muszą być DESC po id');
  }
});

// === getRuns({ status }) + getRunStatusCounts ===

test('status filtruje po dokładnym statusie i łączy się z job_id', () => {
  // Arrange — dwa joby, mieszanka statusów
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '0 9 * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '0 9 * * *' });
  seedRunWith(jobA.id, 'success');
  const failA = seedRunWith(jobA.id, 'failed');
  seedRunWith(jobA.id, 'timeout');
  seedRunWith(jobB.id, 'failed');

  // Act — oba filtry naraz
  const rows = db.getRuns({ job_id: jobA.id, status: 'failed', limit: 100 });

  // Assert — przecięcie, nie suma
  assert.equal(rows.length, 1, 'filtry job_id i status muszą się składać, nie wykluczać');
  assert.equal(rows[0].id, failA);
});

test('status działa też bez job_id (przekrój wszystkich jobów)', () => {
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '0 9 * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '0 9 * * *' });
  seedRunWith(jobA.id, 'timeout');
  seedRunWith(jobB.id, 'timeout');
  seedRunWith(jobB.id, 'success');

  const rows = db.getRuns({ status: 'timeout', limit: 100 });
  assert.equal(rows.length, 2);
  assert.ok(rows.every(r => r.status === 'timeout'));
});

test('status składa się z hideRoutine — udany run rutynowego joba zostaje ukryty', () => {
  const routine = db.createJob({ name: 'inbox', skill_name: 's', cron_expr: '*/1 * * * *', routine: 1 });
  const normal = db.createJob({ name: 'report', skill_name: 's', cron_expr: '0 9 * * *' });
  seedRunWith(routine.id, 'success');
  const okNormal = seedRunWith(normal.id, 'success');

  const rows = db.getRuns({ status: 'success', hideRoutine: true, limit: 100 });
  assert.deepEqual(rows.map(r => r.id), [okNormal], 'hideRoutine musi obowiązywać także przy filtrze statusu');
});

test('getRunStatusCounts liczy CAŁĄ bazę, nie okno limitu', () => {
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '0 9 * * *' });
  seedRuns(job.id, 120); // 120 sukcesów — więcej niż domyślne okno historii (100)
  seedRunWith(job.id, 'failed');

  const stats = db.getRunStatusCounts();
  assert.equal(stats.by_status.success, 120, 'licznik nie może być przycięty limitem listy');
  assert.equal(stats.by_status.failed, 1);
  assert.equal(stats.total, 121);
});

test('getRunStatusCounts respektuje job_id i hideRoutine', () => {
  const routine = db.createJob({ name: 'inbox', skill_name: 's', cron_expr: '*/1 * * * *', routine: 1 });
  const normal = db.createJob({ name: 'report', skill_name: 's', cron_expr: '0 9 * * *' });
  seedRunWith(routine.id, 'success');
  seedRunWith(routine.id, 'failed');
  seedRunWith(normal.id, 'success');

  const hidden = db.getRunStatusCounts({ hideRoutine: true });
  assert.equal(hidden.by_status.success, 1, 'sukces rutynowego joba nie liczy się przy hideRoutine');
  assert.equal(hidden.by_status.failed, 1);

  const perJob = db.getRunStatusCounts({ job_id: routine.id });
  assert.equal(perJob.total, 2, 'wybrany job pokazuje wszystkie swoje runy, także rutynowy sukces');
});

test('getRunStatusCounts: brak runów → zera, nie undefined', () => {
  const stats = db.getRunStatusCounts();
  assert.equal(stats.total, 0);
  assert.deepEqual(stats.by_status, {});
});

test('licznik pill-a zgadza się z długością listy przy tych samych filtrach', () => {
  // Regresja na rozjazd, o który cała ta zmiana się opiera: pill mówi 12, lista pokazuje 9.
  const routine = db.createJob({ name: 'inbox', skill_name: 's', cron_expr: '*/1 * * * *', routine: 1 });
  const normal = db.createJob({ name: 'report', skill_name: 's', cron_expr: '0 9 * * *' });
  for (let i = 0; i < 5; i += 1) seedRunWith(routine.id, 'failed');
  for (let i = 0; i < 3; i += 1) seedRunWith(normal.id, 'failed');
  seedRunWith(routine.id, 'success');

  const stats = db.getRunStatusCounts({ hideRoutine: true });
  const rows = db.getRuns({ status: 'failed', hideRoutine: true, limit: 1000 });
  assert.equal(rows.length, stats.by_status.failed);
});

test('RUN_STATUSES pokrywa się z mapą etykiet frontendu (enum-map.js)', () => {
  // Backend waliduje whitelistą, frontend renderuje pill-e z własnej mapy — rozjazd
  // objawiłby się jako 400 na kliknięcie istniejącego statusu albo pill bez etykiety.
  const { mapStatus, STATUS_ORDER } = require('../public/enum-map.js');
  for (const status of db.RUN_STATUSES) {
    assert.notEqual(mapStatus(status).label, 'Nieznany', `frontend nie zna statusu ${status}`);
  }
  // Druga strona: status dołożony TYLKO na froncie da pill, którego backend odrzuci z 400.
  assert.deepEqual([...STATUS_ORDER].sort(), [...db.RUN_STATUSES].sort(), 'listy statusów muszą być identyczne w obie strony');
});

// === getRuns({ fields: 'meta' }) ===

// Pomocnik: run z realnym logiem (seedRunWith wstawia same metadane).
function seedRunWithLog(jobId, stdout, stderr = '') {
  const conn = db.getDb();
  const result = conn
    .prepare("INSERT INTO runs (job_id, status, trigger_type, stdout, stderr) VALUES (?, 'success', 'scheduled', ?, ?)")
    .run(jobId, stdout, stderr);
  return result.lastInsertRowid;
}

test('fields=meta zwraca metadane bez logów, z rozmiarami w bajtach', () => {
  // Arrange — log z polskim znakiem: bajty ≠ znaki (UTF-8)
  const job = db.createJob({ name: 'meta', skill_name: 's', cron_expr: '0 9 * * *' });
  const runId = seedRunWithLog(job.id, 'ó', 'err');

  // Act
  const row = db.getRuns({ fields: 'meta', limit: 100 }).find(r => r.id === runId);

  // Assert — zero ciężkich pól, rozmiary policzone w bajtach
  assert.ok(row, 'run musi być na liście');
  assert.equal(row.stdout, undefined, 'stdout nie może wyjść w trybie meta');
  assert.equal(row.stderr, undefined, 'stderr nie może wyjść w trybie meta');
  assert.equal(row.webhook_payload, undefined, 'webhook_payload nie może wyjść w trybie meta');
  assert.equal(row.stdout_bytes, 2, 'ó to 2 bajty UTF-8');
  assert.equal(row.stderr_bytes, 3);
  assert.equal(typeof row.stdout_bytes, 'number', 'agregaty node:sqlite bywają BigInt — kontrakt to number');
  assert.equal(row.status, 'success');
  assert.equal(row.job_id, job.id);
});

test('bez fields lista zwraca pełne logi (kontrakt istniejących konsumentów)', () => {
  // Arrange
  const job = db.createJob({ name: 'full', skill_name: 's', cron_expr: '0 9 * * *' });
  const runId = seedRunWithLog(job.id, 'pełny log');

  // Act
  const row = db.getRuns({ limit: 100 }).find(r => r.id === runId);

  // Assert
  assert.equal(row.stdout, 'pełny log');
  assert.equal(row.stdout_bytes, undefined, 'tryb pełny nie dokłada pól meta');
});

test('fields=meta działa razem z job_id i hideRoutine', () => {
  // Arrange — rutynowy job z fail + normalny job
  const routine = db.createJob({ name: 'inbox-meta', skill_name: 's', cron_expr: '*/1 * * * *', routine: 1 });
  const okId = seedRunWith(routine.id, 'success');
  const failId = seedRunWith(routine.id, 'failed');

  // Act
  const byJob = db.getRuns({ fields: 'meta', job_id: routine.id, limit: 100 });
  const hidden = db.getRuns({ fields: 'meta', hideRoutine: true, limit: 100 }).map(r => r.id);

  // Assert
  assert.equal(byJob.length, 2, 'job_id filtruje w trybie meta');
  assert.ok(byJob.every(r => r.stdout === undefined), 'meta bez logów także przy job_id');
  assert.ok(!hidden.includes(okId), 'hideRoutine działa w trybie meta');
  assert.ok(hidden.includes(failId));
});

// === deleteOldRoutineRuns ===

test('deleteOldRoutineRuns kasuje tylko stare SUCCESS rutynowych jobów', () => {
  // Arrange
  const routine = db.createJob({ name: 'inbox', skill_name: 's', cron_expr: '*/1 * * * *', routine: 1 });
  const normal = db.createJob({ name: 'report', skill_name: 's', cron_expr: '0 9 * * *' });

  const oldSuccess = seedRunWith(routine.id, 'success', hoursAgoIso(48)); // kasowany
  const recentSuccess = seedRunWith(routine.id, 'success', hoursAgoIso(1)); // zostaje (świeży)
  const oldFail = seedRunWith(routine.id, 'failed', hoursAgoIso(48)); // zostaje (nie success)
  const oldTimeout = seedRunWith(routine.id, 'timeout', hoursAgoIso(48)); // zostaje
  const normalOldSuccess = seedRunWith(normal.id, 'success', hoursAgoIso(48)); // zostaje (nierutynowy)

  // Act — cutoff 24h
  const deleted = db.deleteOldRoutineRuns(24);

  // Assert — usunięto dokładnie 1 (stary success rutynowego)
  assert.equal(deleted, 1);
  const remainingIds = db.getRuns({ limit: 100 }).map(r => r.id);
  assert.ok(!remainingIds.includes(oldSuccess), 'stary success rutynowego skasowany');
  assert.ok(remainingIds.includes(recentSuccess), 'świeży success zostaje');
  assert.ok(remainingIds.includes(oldFail), 'fail zostaje na zawsze');
  assert.ok(remainingIds.includes(oldTimeout), 'timeout zostaje na zawsze');
  assert.ok(remainingIds.includes(normalOldSuccess), 'success nierutynowego joba zostaje');
});

// === reapOrphanedRuns ===

test('reapOrphanedRuns oznacza osierocony running jako killed z finished_at i error_msg', () => {
  // Arrange — run 'running' bez finished_at (jak po przerwanym procesie)
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const orphanId = seedRunWith(job.id, 'running', null);

  // Act
  const reaped = db.reapOrphanedRuns();

  // Assert — zwrotka to lista zebranych {id, job_id} (Unit 6: start serwera rozpoznaje
  // po job_id runy teczki ask i wysyła ❌ o restarcie)
  assert.deepEqual(reaped, [{ id: orphanId, job_id: job.id }]);
  const row = db.getDb().prepare('SELECT status, finished_at, error_msg FROM runs WHERE id = ?').get(orphanId);
  assert.equal(row.status, 'killed');
  assert.ok(row.finished_at, 'finished_at musi być ustawione');
  assert.equal(row.error_msg, 'Przerwany — restart serwera');
});

test('reapOrphanedRuns nie tyka runów zakończonych ani w kolejce', () => {
  // Arrange — success, failed, queued (żaden nie jest osierocony)
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const okId = seedRunWith(job.id, 'success', hoursAgoIso(1));
  const failId = seedRunWith(job.id, 'failed', hoursAgoIso(1));
  const queuedId = seedRunWith(job.id, 'queued', null);

  // Act
  const reaped = db.reapOrphanedRuns();

  // Assert — nic nie naprawiono (pusta lista), statusy nietknięte
  assert.deepEqual(reaped, []);
  const conn = db.getDb();
  assert.equal(conn.prepare('SELECT status FROM runs WHERE id = ?').get(okId).status, 'success');
  assert.equal(conn.prepare('SELECT status FROM runs WHERE id = ?').get(failId).status, 'failed');
  assert.equal(conn.prepare('SELECT status FROM runs WHERE id = ?').get(queuedId).status, 'queued');
});

test('reapOrphanedRuns gasi getCurrentRun (kill-bar już nie wisi)', () => {
  // Arrange — osierocony running powoduje, że getCurrentRun coś zwraca
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunWith(job.id, 'running', null);
  assert.ok(db.getCurrentRun(), 'przed reaperem kill-bar ma co pokazać');

  // Act
  db.reapOrphanedRuns();

  // Assert — brak running → kill-bar pusty
  assert.equal(db.getCurrentRun(), null);
});

test('reapOrphanedRuns zwraca job_id każdego zebranego runu (osierocone z dwóch jobów)', () => {
  // Arrange — po dwa różne joby z osieroconym runem; start serwera filtruje po job_id
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '*/1 * * * *' });
  const orphanA = seedRunWith(jobA.id, 'running', null);
  const orphanB = seedRunWith(jobB.id, 'running', null);

  // Act
  const reaped = db.reapOrphanedRuns();

  // Assert — każdy zebrany run niesie swój job_id
  assert.deepEqual(
    reaped.sort((a, b) => a.id - b.id),
    [{ id: orphanA, job_id: jobA.id }, { id: orphanB, job_id: jobB.id }]
  );
});

// === run_on_wake default (R1) ===

test('createJob bez run_on_wake → run_on_wake === 1 (opt-out default)', () => {
  // Arrange + Act
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });

  // Assert
  assert.equal(job.run_on_wake, 1);
});

test('createJob({ run_on_wake: 0 }) → run_on_wake === 0 (opt-out działa)', () => {
  // Arrange + Act
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *', run_on_wake: 0 });

  // Assert
  assert.equal(job.run_on_wake, 0);
});

// === wake_backfill jednorazowy (R2) ===

test('backfill ustawia istniejące joby na run_on_wake=1 i flagę wake_backfill_done', () => {
  // Arrange — symuluj stan sprzed backfillu: joby z run_on_wake=0 i brak flagi.
  const conn = db.getDb();
  const a = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const b = db.createJob({ name: 'B', skill_name: 's', cron_expr: '0 9 * * *' });
  conn.exec('UPDATE jobs SET run_on_wake = 0');
  conn.exec("DELETE FROM state WHERE key = 'wake_backfill_done'");

  // Act — ponowny migrate na tym samym połączeniu
  db.migrate(conn);

  // Assert — oba joby podniesione na 1, flaga ustawiona
  assert.equal(db.getJob(a.id).run_on_wake, 1);
  assert.equal(db.getJob(b.id).run_on_wake, 1);
  assert.equal(db.getState('wake_backfill_done'), '1');
});

test('backfill jest jednorazowy — po opt-oucie kolejny migrate nie clobberuje (flaga blokuje)', () => {
  // Arrange — pierwszy backfill (flagi brak), potem user ręcznie wyłącza run_on_wake.
  const conn = db.getDb();
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  conn.exec('UPDATE jobs SET run_on_wake = 0');
  conn.exec("DELETE FROM state WHERE key = 'wake_backfill_done'");
  db.migrate(conn);
  assert.equal(db.getJob(job.id).run_on_wake, 1, 'pierwszy backfill podnosi na 1');

  // user opt-out
  db.updateJob(job.id, { run_on_wake: 0 });

  // Act — kolejny migrate (flaga już ustawiona)
  db.migrate(conn);

  // Assert — opt-out NIE jest clobberowany
  assert.equal(db.getJob(job.id).run_on_wake, 0);
});

// === CASCADE delete ===

test('deleteJob kasuje też wszystkie runy tego joba (ON DELETE CASCADE)', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRuns(job.id, 4);
  assert.equal(db.getRuns({ job_id: job.id, limit: 100 }).length, 4);

  // Act
  db.deleteJob(job.id);

  // Assert — runy zniknęły wraz z jobem
  assert.equal(db.getRuns({ job_id: job.id, limit: 100 }).length, 0);
  assert.equal(db.getJob(job.id), undefined);
});

// === telegram_notify (Faza 2 — kanał Telegram, R1) ===

test('createJob({ telegram_notify: true }) → 1 w bazie; bez flagi → default 0', () => {
  // Arrange + Act
  const withFlag = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *', telegram_notify: true });
  const withoutFlag = db.createJob({ name: 'B', skill_name: 's', cron_expr: '*/1 * * * *' });

  // Assert — koercja bool→int przy insert, default 0
  assert.equal(withFlag.telegram_notify, 1);
  assert.equal(withoutFlag.telegram_notify, 0);
});

test('updateJob koerca telegram_notify: false→0, true→1', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *', telegram_notify: true });

  // Act + Assert
  assert.equal(db.updateJob(job.id, { telegram_notify: false }).telegram_notify, 0);
  assert.equal(db.updateJob(job.id, { telegram_notify: true }).telegram_notify, 1);
});

test('stara baza bez telegram_notify dostaje kolumnę po migracji z defaultem 0', () => {
  // Arrange — świeże połączenie ze schematem jobs sprzed Fazy 2 (bez telegram_notify),
  // z istniejącym jobem. Pozostałe tabele utworzy sam migrate (IF NOT EXISTS).
  const { DatabaseSync } = require('node:sqlite');
  const old = new DatabaseSync(':memory:');
  old.exec(`
    CREATE TABLE jobs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      name TEXT NOT NULL,
      skill_name TEXT NOT NULL,
      cron_expr TEXT NOT NULL,
      arguments TEXT DEFAULT '',
      enabled INTEGER DEFAULT 1,
      run_on_wake INTEGER DEFAULT 1,
      timeout_ms INTEGER DEFAULT 600000,
      max_retries INTEGER DEFAULT 1,
      discord_notify INTEGER DEFAULT 0,
      created_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT DEFAULT (datetime('now'))
    );
  `);
  old.prepare("INSERT INTO jobs (name, skill_name, cron_expr) VALUES ('legacy', 's', '*/1 * * * *')").run();

  // Act — migracja jak przy boocie na starej bazie
  db.migrate(old);

  // Assert — kolumna istnieje, istniejący job dostał default 0
  const cols = old.prepare('PRAGMA table_info(jobs)').all().map(c => c.name);
  assert.ok(cols.includes('telegram_notify'), 'migracja musi dodać kolumnę telegram_notify');
  const row = old.prepare("SELECT telegram_notify FROM jobs WHERE name = 'legacy'").get();
  assert.equal(row.telegram_notify, 0);
  old.close();
});

// === Smoke-test typów DB (R4) ===

test('assertDbReturnsNumbers jest wyeksportowany', () => {
  assert.equal(typeof db.assertDbReturnsNumbers, 'function');
});

test('połączenie ma busy_timeout=5000 — rywalizacja o zapis czeka zamiast crashować (N4 07.08)', () => {
  const conn = db.getDb();
  assert.equal(conn.prepare('PRAGMA busy_timeout').get().timeout, 5000);
});

test('smoke-test przechodzi na realnej :memory: DB (COUNT zwraca number)', () => {
  // Arrange — realna baza in-memory z migracją (from before()).
  const conn = db.getDb();

  // Act + Assert — nie rzuca.
  assert.doesNotThrow(() => db.assertDbReturnsNumbers(conn));
});

test('smoke-test rzuca DbTypeError gdy agregat nie jest number (error case R4)', () => {
  // Arrange — stub connection symulujący node:sqlite zwracający BigInt/string zamiast number.
  const badConn = {
    prepare() {
      return { get: () => ({ n: '0' }) }; // string zamiast number
    },
  };

  // Act + Assert — typed error z czytelnym komunikatem.
  assert.throws(
    () => db.assertDbReturnsNumbers(badConn),
    (err) => {
      assert.ok(err instanceof db.DbTypeError);
      assert.match(err.message, /number/);
      return true;
    }
  );
});

// === countRecentFailedRuns (wspólne okno retry/R9 dla scheduler.processQueue i executor.notifyRunOutcome) ===

test('countRecentFailedRuns: liczy TYLKO faile w oknie ostatnich max_retries+1 runów', () => {
  // Arrange — 3 stare faile poza oknem, potem success + fail w oknie (max_retries=1 → okno 2)
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const conn = db.getDb();
  const insert = conn.prepare("INSERT INTO runs (job_id, status, trigger_type) VALUES (?, ?, 'scheduled')");
  insert.run(job.id, 'failed');
  insert.run(job.id, 'failed');
  insert.run(job.id, 'failed');
  insert.run(job.id, 'success');
  insert.run(job.id, 'failed');

  // Act
  const count = db.countRecentFailedRuns(job.id, 1);

  // Assert — okno = 2 ostatnie runy (success + failed) → 1 fail, stare faile niewliczone
  assert.equal(count, 1);
});

test('countRecentFailedRuns: wszystkie runy w oknie failed → pełne okno (próg final fail R9)', () => {
  // Arrange — max_retries=1, oryginał + retry oba failed
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const insert = db.getDb().prepare("INSERT INTO runs (job_id, status, trigger_type) VALUES (?, ?, 'scheduled')");
  insert.run(job.id, 'failed');
  insert.run(job.id, 'failed');

  // Act / Assert — 2 > max_retries=1 → executor uzna fail za ostateczny
  assert.equal(db.countRecentFailedRuns(job.id, 1), 2);
});

test('countRecentFailedRuns: brak runów / maxRetries falsy → 0 bez wyjątku (error case)', () => {
  // Arrange — job bez runów; maxRetries undefined traktowane jak 0 (okno = 1)
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });

  // Act / Assert
  assert.equal(db.countRecentFailedRuns(job.id, undefined), 0);
  assert.equal(db.countRecentFailedRuns(job.id, 0), 0);
});

// === Równoległe joby, Unit 1: lock_group, queued_at, runy aktywne, próbki czasów ===

// Wstawia run z pełną kontrolą znaczników czasu (queued_at/started_at/finished_at jako ISO albo null).
function seedRunTimed(jobId, status, { queued_at = null, started_at = null, finished_at = null } = {}) {
  const result = db.getDb()
    .prepare(
      "INSERT INTO runs (job_id, status, trigger_type, queued_at, started_at, finished_at) VALUES (?, ?, 'scheduled', ?, ?, ?)"
    )
    .run(jobId, status, queued_at, started_at, finished_at);
  return result.lastInsertRowid;
}

function isoAgo(ms) {
  return new Date(Date.now() - ms).toISOString();
}

test('migrate() dwa razy na starej bazie nie rzuca i nie duplikuje lock_group/queued_at', () => {
  // Arrange — schemat sprzed tego sprintu: jobs bez lock_group, runs bez queued_at.
  const { DatabaseSync } = require('node:sqlite');
  const old = new DatabaseSync(':memory:');
  old.exec(`
    CREATE TABLE jobs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      name TEXT NOT NULL,
      skill_name TEXT NOT NULL,
      cron_expr TEXT NOT NULL,
      enabled INTEGER DEFAULT 1,
      run_on_wake INTEGER DEFAULT 1
    );
    CREATE TABLE runs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      job_id INTEGER NOT NULL,
      status TEXT NOT NULL DEFAULT 'queued',
      trigger_type TEXT NOT NULL DEFAULT 'scheduled',
      started_at TEXT,
      finished_at TEXT
    );
  `);
  old.prepare("INSERT INTO jobs (name, skill_name, cron_expr) VALUES ('legacy', 's', '*/1 * * * *')").run();
  old.prepare("INSERT INTO runs (job_id, status, started_at) VALUES (1, 'success', '2026-07-01T10:00:00.000Z')").run();

  // Act — migracja jak przy dwóch kolejnych bootach
  assert.doesNotThrow(() => db.migrate(old));
  assert.doesNotThrow(() => db.migrate(old));

  // Assert — po jednej kolumnie każdego rodzaju, stary run BEZ backfillu queued_at
  const jobCols = old.prepare('PRAGMA table_info(jobs)').all().map(c => c.name);
  const runCols = old.prepare('PRAGMA table_info(runs)').all().map(c => c.name);
  assert.equal(jobCols.filter(c => c === 'lock_group').length, 1);
  assert.equal(runCols.filter(c => c === 'queued_at').length, 1);
  assert.equal(old.prepare("SELECT queued_at FROM runs WHERE id = 1").get().queued_at, null);
  old.close();
});

test('createJob zapisuje lock_group, updateJob ją zmienia i czyści (R5)', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *', lock_group: 'dashboard' });
  const bare = db.createJob({ name: 'B', skill_name: 's', cron_expr: '*/1 * * * *' });

  // Act + Assert
  assert.equal(job.lock_group, 'dashboard');
  assert.equal(bare.lock_group, null, 'bez deklaracji job nie należy do żadnej grupy');
  assert.equal(db.updateJob(job.id, { lock_group: 'vault' }).lock_group, 'vault');
  assert.equal(db.updateJob(job.id, { lock_group: null }).lock_group, null);
});

test('createRun ustawia queued_at (ISO UTC, ten sam format co started_at)', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const before = Date.now();

  // Act
  const run = db.createRun({ job_id: job.id });

  // Assert — parsowalny ISO z chwili wstawienia (tolerancja na czas wykonania testu)
  assert.ok(run.queued_at, 'queued_at musi być ustawione przy tworzeniu runu');
  const parsed = Date.parse(run.queued_at);
  assert.ok(Number.isFinite(parsed), 'queued_at musi być parsowalny przez Date.parse');
  assert.ok(parsed >= before - 1000 && parsed <= Date.now() + 1000);
});

test('getQueueWaitStats liczy czekanie w kolejce i pomija runy bez queued_at (R8)', () => {
  // Arrange — dwa zmierzone runy (5 s i 1 s czekania) + run sprzed migracji (queued_at NULL)
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunTimed(job.id, 'success', { queued_at: isoAgo(65000), started_at: isoAgo(60000) });
  seedRunTimed(job.id, 'success', { queued_at: isoAgo(31000), started_at: isoAgo(30000) });
  seedRunTimed(job.id, 'success', { queued_at: null, started_at: isoAgo(10000) });

  // Act
  const stats = db.getQueueWaitStats(24);

  // Assert — legacy run niewliczony (inaczej średnia spadłaby do 2000 ms)
  assert.equal(stats.count, 2);
  assert.equal(stats.avgMs, 3000);
  assert.equal(stats.maxMs, 5000);
  assert.equal(typeof stats.avgMs, 'number');
  assert.equal(typeof stats.maxMs, 'number');
});

test('getQueueWaitStats pomija runy spoza okna i bez startu → zera zamiast NaN (error case)', () => {
  // Arrange — run sprzed 48 h oraz run wciąż w kolejce (bez started_at); okno = 24 h
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunTimed(job.id, 'success', { queued_at: isoAgo(48 * 3600000 + 5000), started_at: isoAgo(48 * 3600000) });
  seedRunTimed(job.id, 'queued', { queued_at: isoAgo(1000) });

  // Act
  const stats = db.getQueueWaitStats(24);

  // Assert
  assert.deepEqual(stats, { count: 0, avgMs: 0, maxMs: 0 });
});

test('getRunningRuns zwraca WSZYSTKIE aktywne runy, getCurrentRun dalej jeden (R2)', () => {
  // Arrange — dwa równoległe runy + jeden zakończony
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '*/1 * * * *' });
  const idA = seedRunTimed(jobA.id, 'running', { queued_at: isoAgo(5000), started_at: isoAgo(4000) });
  const idB = seedRunTimed(jobB.id, 'running', { queued_at: isoAgo(3000), started_at: isoAgo(2000) });
  seedRunTimed(jobA.id, 'success', { started_at: isoAgo(9000), finished_at: isoAgo(8000) });

  // Act
  const running = db.getRunningRuns();

  // Assert — pełny obraz równoległości; stary call-site dostaje nadal pojedynczy run
  assert.deepEqual(running.map(r => r.id), [idA, idB]);
  assert.ok(db.getCurrentRun());
  assert.equal(typeof db.getCurrentRun().id, 'number');
});

test('getRunningRuns bez aktywnych runów → pusta tablica (nie null)', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunTimed(job.id, 'success', { started_at: isoAgo(9000), finished_at: isoAgo(8000) });

  // Act + Assert
  assert.deepEqual(db.getRunningRuns(), []);
  assert.equal(db.getCurrentRun(), null);
});

test('getRunningRuns zwraca metadane bez logów i bez webhook_payload (R1)', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const id = seedRunTimed(job.id, 'running', { queued_at: isoAgo(5000), started_at: isoAgo(4000) });

  // Act
  const [run] = db.getRunningRuns();

  // Assert — /api/status jest odpytywany cyklicznie, więc wozi wyłącznie metadane
  assert.ok(!('stdout' in run), 'stdout nie ma prawa wejść do payloadu statusu');
  assert.ok(!('stderr' in run), 'stderr nie ma prawa wejść do payloadu statusu');
  assert.ok(!('webhook_payload' in run), 'webhook_payload nie ma prawa wejść do payloadu statusu');
  assert.equal(run.id, id);
  assert.equal(run.job_id, job.id);
  assert.equal(run.status, 'running');
  assert.equal(run.trigger_type, 'scheduled');
  assert.ok(run.queued_at, 'queued_at zostaje — kolejka i UI liczą z niego czekanie');
  assert.ok(run.started_at, 'started_at zostaje — kill-bar liczy z niego czas trwania');
});

test('getRunningRuns nie wypuszcza webhook_payload biegnącego runu webhookowego (error case)', () => {
  // Arrange — payload webhooka to jedyne pole bez capa rozmiaru (parseBody go nie ogranicza)
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const created = db.createRun({ job_id: job.id, trigger_type: 'webhook', webhook_payload: '{"sekret":"nie-do-statusu"}' });
  db.updateRun(created.id, { status: 'running', started_at: isoAgo(1000) });

  // Act
  const running = db.getRunningRuns();

  // Assert — payload dalej siedzi w bazie (prompt joba go potrzebuje), ale nie w odpowiedzi
  assert.equal(running.length, 1);
  assert.ok(!('webhook_payload' in running[0]));
  assert.ok(!JSON.stringify(running).includes('nie-do-statusu'), 'payload nie może wyciec żadną kolumną');
});

test('getRunningRuns: dwa biegnące runy zachowują kolejność id ASC (current_run = pierwszy)', () => {
  // Arrange — starszy run ma niższe id, ale późniejszy started_at (kolejność wisi na id)
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '*/1 * * * *' });
  const first = seedRunTimed(jobA.id, 'running', { started_at: isoAgo(1000) });
  const second = seedRunTimed(jobB.id, 'running', { started_at: isoAgo(9000) });

  // Act + Assert
  assert.deepEqual(db.getRunningRuns().map(r => r.id), [first, second]);
});

test('getRecentSuccessDurations: tylko udane runy z kompletem znaczników, w ms jako number (R3)', () => {
  // Arrange — 1 udany kompletny + fail/timeout/killed + udany bez started_at
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunTimed(job.id, 'success', { started_at: isoAgo(20000), finished_at: isoAgo(8000) }); // 12 s
  seedRunTimed(job.id, 'failed', { started_at: isoAgo(20000), finished_at: isoAgo(19000) });
  seedRunTimed(job.id, 'timeout', { started_at: isoAgo(20000), finished_at: isoAgo(19000) });
  seedRunTimed(job.id, 'killed', { started_at: isoAgo(20000), finished_at: isoAgo(19000) });
  seedRunTimed(job.id, 'success', { started_at: null, finished_at: isoAgo(7000) });

  // Act
  const durations = db.getRecentSuccessDurations(job.id);

  // Assert — jedna próbka, liczba (nie BigInt, nie string)
  assert.deepEqual(durations, [12000]);
  assert.equal(typeof durations[0], 'number');
});

test('getRecentSuccessDurations: limit przycina do N najnowszych, obcy job nie miesza próbek', () => {
  // Arrange — 3 udane runy joba A (1 s, 2 s, 3 s wg rosnącego id) + run joba B
  const jobA = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  const jobB = db.createJob({ name: 'B', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunTimed(jobA.id, 'success', { started_at: isoAgo(30000), finished_at: isoAgo(29000) });
  seedRunTimed(jobA.id, 'success', { started_at: isoAgo(20000), finished_at: isoAgo(18000) });
  seedRunTimed(jobA.id, 'success', { started_at: isoAgo(10000), finished_at: isoAgo(7000) });
  seedRunTimed(jobB.id, 'success', { started_at: isoAgo(10000), finished_at: isoAgo(1000) });

  // Act
  const durations = db.getRecentSuccessDurations(jobA.id, 2);

  // Assert — dwa NAJNOWSZE runy (ORDER BY id DESC), bez próbki joba B (9000)
  assert.deepEqual(durations, [3000, 2000]);
});

test('getRecentSuccessDurations: job bez udanych runów → pusta tablica (error case)', () => {
  // Arrange
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/1 * * * *' });
  seedRunTimed(job.id, 'failed', { started_at: isoAgo(20000), finished_at: isoAgo(19000) });

  // Act + Assert
  assert.deepEqual(db.getRecentSuccessDurations(job.id), []);
});

// === vault (etykieta przestrzeni roboczej) ===

test('createJob bez vault → pusty string, nie NULL (filtr w UI porównuje stringi)', () => {
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/5 * * * *' });
  assert.equal(job.vault, '');
});

test('createJob z jawnym vault → zapisane dokładnie jak podano', () => {
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/5 * * * *', vault: 'notatki-robocze' });
  assert.equal(job.vault, 'notatki-robocze');
});

test('updateJob może zmienić i wyczyścić vault; null normalizowany do pustego stringa', () => {
  const job = db.createJob({ name: 'A', skill_name: 's', cron_expr: '*/5 * * * *', vault: 'praca' });
  assert.equal(db.updateJob(job.id, { vault: 'dom' }).vault, 'dom');
  assert.equal(db.updateJob(job.id, { vault: '' }).vault, '', 'zdjęcie przypisania musi być możliwe');
  // Klient API może przysłać null; baza ma znać JEDEN rodzaj „braku przypisania".
  assert.equal(db.updateJob(job.id, { vault: null }).vault, '');
});

test('migracja starej bazy: kolumna dochodzi, istniejące zadania dostają pusty vault (ZERO zgadywania)', () => {
  // Przypisanie zna tylko właściciel instalacji — heurystyka po ścieżce `command`
  // trafiałaby losowo, a źle zgadnięty sejf jest gorszy niż brak przypisania.
  const { DatabaseSync } = require('node:sqlite');
  const old = new DatabaseSync(':memory:');
  old.exec(`
    CREATE TABLE jobs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      name TEXT NOT NULL,
      skill_name TEXT NOT NULL,
      cron_expr TEXT NOT NULL,
      arguments TEXT DEFAULT '',
      enabled INTEGER DEFAULT 1,
      run_on_wake INTEGER DEFAULT 1,
      timeout_ms INTEGER DEFAULT 600000,
      max_retries INTEGER DEFAULT 1,
      discord_notify INTEGER DEFAULT 0,
      telegram_notify INTEGER DEFAULT 0,
      job_type TEXT DEFAULT 'claude',
      command TEXT,
      routine INTEGER DEFAULT 0,
      created_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT DEFAULT (datetime('now'))
    );
  `);
  old.prepare("INSERT INTO jobs (name, skill_name, cron_expr, job_type, command) VALUES ('sync', '', '*/5 * * * *', 'script', '/opt/scripts/sync.mjs')").run();
  old.prepare("INSERT INTO jobs (name, skill_name, cron_expr) VALUES ('raport', 'daily-report', '0 9 * * *')").run();

  db.migrate(old);

  const cols = old.prepare('PRAGMA table_info(jobs)').all().map(c => c.name);
  assert.ok(cols.includes('vault'), 'migracja musi dodać kolumnę vault');
  for (const name of ['sync', 'raport']) {
    assert.equal(old.prepare('SELECT vault FROM jobs WHERE name = ?').get(name).vault, '', `${name}: brak przypisania, nie zgadnięte`);
  }
  old.close();
});
