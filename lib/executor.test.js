...
// Atrapa martwego procesu wg FAKTYCZNEGO kontraktu Node (zweryfikowanego empirycznie):
// kill() po zakończeniu procesu zwraca `false` i NIE rzuca. Poprzednia wersja tych testów
// stała na atrapie, której kill() rzucał błąd systemowy — czyli na zachowaniu, którego
// Node w tej ścieżce nie ma (wyjątek leci wyłącznie przy nieznanym sygnale).
function deadProc() {
  const signals = [];
  const listeners = { exit: [] };
  return {
    ...
    signals,
    killed: false,
    kill(sig) { signals.push(sig); return false; },
    once(event, fn) { (listeners[event] || (listeners[event] = [])).push(fn); },
    emit(event) { for (const fn of listeners[event] || []) fn(); },
  };
}

test('killProcessTree: martwy proces (kill zwraca false) nie wywraca eskalacji ani nie loguje', (t) => {
  if (process.platform === 'win32') return;
  t.mock.timers.enable({ apis: ['setTimeout'] });
  const warnings = [];
  ...
  assert.deepEqual(proc.signals, ['SIGTERM', 'SIGKILL'], 'eskalacja leci normalnie — kill nie sygnalizuje błędu');
  // Guard regresji, nie asercja kontraktu: dziś w tej ścieżce nie ma żadnego console.warn,
  // więc ta linia nie może upaść — pilnuje wyłącznie tego, żeby logowanie tu nie wróciło.
  assert.deepEqual(warnings, [], 'martwy proces to normalny wyścig, nie zdarzenie do logu');
});

test('killProcessTree: martwy proces zgłaszający exit kasuje uzbrojony SIGKILL', (t) => {
  if (process.platform === 'win32') return;
  t.mock.timers.enable({ apis: ['setTimeout'] });
  const proc = deadProc();
  ...
  t.mock.timers.tick(KILL_ESCALATION_MS + 2);
  assert.deepEqual(proc.signals, ['SIGTERM'], 'po exit nie strzelamy w PID, który może być już cudzy');
});
