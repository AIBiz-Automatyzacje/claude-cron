...
function killProcessTree(proc) {
  ...
  const escalation = setTimeout(() => signalQuietly(proc, 'SIGKILL'), KILL_ESCALATION_MS);
  escalation.unref();
...
// Sygnał „po cichu" znaczy: milczymy WYŁĄCZNIE o ESRCH (proces już nie żyje — normalny
// wyścig przy eskalacji). Każdy inny błąd (EPERM: nie nasz proces) idzie do logu; puste
// `catch {}` ukrywałoby fakt, że kill w ogóle nie zadziałał.
function signalQuietly(proc, signal) {
  try {
    proc.kill(signal);
  } catch (err) {
    if (err.code !== 'ESRCH') {
      console.warn(`[executor] ${signal} do PID ${proc.pid} nie przeszedł: ${err.message}`);
    }
  }
}
