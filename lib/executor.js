...
// Liczy faile w tym samym oknie co retry w scheduler.processQueue.
// Wołane PO db.updateRun — bieżący fail jest już w bazie i wlicza się do okna.
function countRecentFailedRuns(job) {
  return db.getRuns({ job_id: job.id, limit: (job.max_retries || 0) + 1 })
    .filter(r => r.status === 'failed').length;
}

// Powiadomienia po zakończeniu runu — fire-and-forget `.catch(() => {})`:
// pad jednego kanału nie blokuje drugiego ani NIGDY nie wpływa na status runu.
function notifyRunOutcome(job, status, { stdout, stderr, errorMsg }) {
  if (status === 'success') {
    if (job.discord_notify) discord.sendNotification(job, stdout).catch(() => {});
    if (job.telegram_notify) telegram.sendNotification(job, stdout).catch(() => {});
    return;
  }
  if (!isFinalFailure(status, job.max_retries, countRecentFailedRuns(job))) return;

  const failure = { status, error_msg: errorMsg, stderr };
  if (job.discord_notify) discord.sendFailureNotification(job, failure).catch(() => {});
  if (job.telegram_notify) telegram.sendFailureNotification(job, failure).catch(() => {});
}
...
function executeScriptRun(run, job, resolve) {
  ...
  proc.on('close', (code) => {
    ...
    const errorMsg = wasKilled ? 'Killed by user' : timedOut ? 'Timeout exceeded' : '';
    db.updateRun(run.id, {
      status,
      finished_at: new Date().toISOString(),
      exit_code: code,
...
module.exports = { executeRun, killCurrent, getCurrentRunId, isRunning, isFinalFailure };
