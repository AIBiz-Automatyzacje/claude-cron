...
function executeScriptRun(run, job, resolve) {
  ...
  // Payload webhooka przez env, nie argv — treść pochodzi z internetu, a argv
  // trafia do listy procesów (widoczne dla innych użytkowników maszyny).
  // Joby `claude` dostają go w promptcie (patrz wyżej); skryptowe czytają
  // process.env.WEBHOOK_PAYLOAD. Puste dla runów z crona i ręcznych.
  const proc = spawn('node', [job.command], {
    cwd: WORKSPACE_DIR,
    env: { ...process.env, WEBHOOK_PAYLOAD: run.webhook_payload || '' },
