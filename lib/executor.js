...
function readOauthToken(filePath) {
  try {
    ...
    if (err.code !== 'ENOENT') {
      console.error(`[executor] Nie mogę odczytać pliku tokena OAuth (${err.code}): ${filePath}`);
    }
    return null;
  }
}
...
function executeRun(run) {
  return new Promise((resolve) => {
    ...
    const oauthToken = readOauthToken(OAUTH_TOKEN_FILE);
    if (oauthToken) {
...
module.exports = { executeRun, killCurrent, getCurrentRunId, isRunning, isFinalFailure, notifyRunOutcome, readOauthToken };
