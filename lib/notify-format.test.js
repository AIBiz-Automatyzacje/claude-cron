...
test("smartSplit: '. ' dokładnie na indeksie maxLen → chunk NIE przekracza maxLen (off-by-one, granica)", () => {
  // Arrange — kropka zaczyna się dokładnie na indeksie maxLen: stary kod robił
  // lastIndexOf('. ', maxLen) → splitAt=maxLen, +1 za kropkę → chunk maxLen+1 znaków.
  const maxLen = 10;
  const text = 'a'.repeat(maxLen) + '. ' + 'b'.repeat(5);

  // Act
  const chunks = smartSplit(text, maxLen);

  // Assert — invariant twardy: KAŻDY chunk ≤ maxLen (dokładnie na granicy, nie maxLen+1)
  for (const chunk of chunks) {
    assert.ok(chunk.length <= maxLen, `chunk przekracza limit: ${chunk.length} > ${maxLen}`);
  }
  assert.equal(chunks.join('').replace(/\s/g, ''), text.replace(/\s/g, ''), 'treść bez strat');
});

test("smartSplit: '. ' na indeksie maxLen-1 → kropka zostaje w chunku o długości dokładnie maxLen", () => {
  // Arrange — pozytywna granica: splitAt=maxLen-1, +1 za kropkę → chunk = maxLen (legalny)
  const maxLen = 10;
  const text = 'a'.repeat(maxLen - 1) + '. ' + 'b'.repeat(5);

  // Act
  const chunks = smartSplit(text, maxLen);

  // Assert
  assert.equal(chunks[0], 'a'.repeat(maxLen - 1) + '.');
  assert.equal(chunks[0].length, maxLen);
});

test('smartSplit: tekst z samych \\n → brak pustych chunków (Telegram 400 na pustym text)', () => {
  // Arrange — repro z review: '\n'.repeat(30) przy limicie 10 dawało [""]
  const chunks = smartSplit('\n'.repeat(30), 10);

  // Assert — zero pustych chunków (tu: pusta lista, nic do wysłania)
  assert.deepEqual(chunks, []);
});

test('smartSplit: maxLen <= 0 → RangeError zamiast nieskończonej pętli (fail-fast)', () => {
  // Act / Assert — ujemny/zerowy/nie-całkowity limit nie może wejść w pętlę dzielenia
  assert.throws(() => smartSplit('x'.repeat(50), 0), RangeError);
  assert.throws(() => smartSplit('x'.repeat(50), -5), RangeError);
  assert.throws(() => smartSplit('x'.repeat(50), 2.5), RangeError);
});
