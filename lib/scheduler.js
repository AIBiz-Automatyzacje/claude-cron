...
// === Karencja po wybudzeniu (R11) ===

// Ile czekamy z pierwszym startem runu po wybudzeniu maszyny.
//
// Po wyjściu z uśpienia interfejsy sieciowe, DNS i Tailscale wstają z opóźnieniem —
// job odpalony w pierwszych sekundach pada na ENOTFOUND, mimo że sam jest sprawny.
// 45 s to środek widełek 30–60 s: krócej niż jeden cykl heartbeatu (czyli karencja nigdy
// nie zjada więcej niż jedno tyknięcie), a dłużej niż typowy czas powrotu DNS-a.
const WAKE_GRACE_MS = 45_000;

// Chwila (ms, zegar Date.now()), w której maszyna wróciła do życia — albo null.
// Trzymana W PAMIĘCI, nie w `state`: to stan bieżącego procesu, a nie dana do przetrwania
// restartu (po restarcie ścieżkę downtime'u wykrywa detectWakeFromDowntime).
let wakeDetectedAt = null;
// Poprzednie tyknięcie heartbeatu — jedyne wiarygodne źródło luki snu przy ŻYJĄCYM procesie
// (patrz komentarz w startHeartbeat: znacznik z DB jest nadpisywany natychmiast po pobudce).
let lastHeartbeatAt = null;

// Pure. Czy przerwa między dwoma śladami życia jest snem maszyny, a nie normalną pracą.
// Próg współdzielony z executor.SLEEP_GAP_MS — w projekcie ma być JEDNA definicja snu
// (tam uzasadnienie, dlaczego minuta, a nie kilkanaście sekund: zadławiony event loop
// wygląda identycznie jak sen, więc niższy próg łapałby zwykłe zadławienia).
...
// Próg snu dla śladów zostawianych CO HEARTBEAT (tyknięcie w pamięci i last_active_at w DB).
...
// „nie wcześniej niż" — realna luka to zawsze 60_00x ms, więc próg równy okresowi brałby
// KAŻDE normalne tyknięcie za wybudzenie i zamrażał kolejkę karencją 45 s co minutę.
// Okres + definicja snu = całe jedno tyknięcie zapasu i ZERO nowej definicji „ile trwa sen".
const WAKE_GAP_MS = HEARTBEAT_INTERVAL_MS + executor.SLEEP_GAP_MS;

// Wykrycie luki wall-clock na ścieżce, która akurat biegnie. Heartbeat i pętla kolejki
// patrzą na TEN SAM ślad (lastHeartbeatAt), więc karencja nie zależy od tego, który zaległy
// timer libuv odpali po pobudce pierwszy — cron joba `* * * * *` typowo wyprzedza heartbeat.
// Zwraca true, gdy wybudzenie właśnie wykryto (dla logu wołającego).
function noteWakeIfGap(now) {
  if (!isWakeGap(lastHeartbeatAt, now, WAKE_GAP_MS)) return false;
  const gapS = Math.round((now - lastHeartbeatAt) / 1000);
  markWakeDetected(now);
  // Przesunięcie śladu jest częścią detekcji: bez niego kolejna iteracja pętli (albo
  // najbliższe tyknięcie) zobaczyłaby tę samą lukę i przedłużała karencję w nieskończoność.
  lastHeartbeatAt = now;
  console.log(`[scheduler] wybudzenie po ${gapS} s przerwy — karencja ${WAKE_GRACE_MS / 1000} s przed startem runów (sieć wstaje wolniej niż scheduler)`);
  return true;
}

// Pure. Ile ms zostało do końca karencji (0 = można startować od razu).
function wakeGraceRemainingMs(wakeAt, now, graceMs = WAKE_GRACE_MS) {
  if (wakeAt === null || wakeAt === undefined) return 0;
  const since = now - wakeAt;
  // Ujemne „since" (zegar cofnięty przez NTP po pobudce) traktujemy jak koniec karencji:
  // lepiej jeden run za wcześnie niż kolejka zamrożona do czasu, aż zegar dogoni znacznik.
  if (!Number.isFinite(since) || since < 0) return 0;
  return since < graceMs ? graceMs - since : 0;
}

// Pure. Decyzja pętli: wstrzymać start runów, bo maszyna dopiero wróciła do życia.
// `wakeAt` to chwila WYBUDZENIA (wykryta luka w heartbeacie albo start po downtimie),
// świadomie NIE `last_active_at` z bazy — heartbeat nadpisuje ten znacznik zaległym
// tyknięciem w milisekundach po pobudce, więc luka znikałaby, zanim ktokolwiek ją przeczyta.
function shouldDeferAfterWake(wakeAt, now, graceMs = WAKE_GRACE_MS) {
  return wakeGraceRemainingMs(wakeAt, now, graceMs) > 0;
}

// Zapis chwili wybudzenia. `null` czyści stan (używane przez testy — wzorzec db.setDbPath).
// Idempotentne: wielokrotne wykrycie tego samego wybudzenia tylko przesuwa znacznik.
function markWakeDetected(atMs = Date.now()) {
  wakeDetectedAt = atMs;
}

function getWakeDetectedAt() {
  return wakeDetectedAt;
}

// Timer karencji. `unref()` — karencja nie może trzymać procesu przy życiu (wzorzec lib/ask.js).
...
function startEligibleRuns(ctx) {
  ...
  // Próbki świadomie N zapytaniami (przy dzisiejszych ~9 włączonych jobach to mikrosekundy);
  // powyżej ~30 jobów zamienić na jedno zapytanie z window function wzorem db.getRecentRunsPerJob.
...
// Czy w kolejce stoi cokolwiek, na co warto czekać do końca karencji. Pytamy TYLKO gdy
// karencja trwa — na normalnej ścieżce to zapytanie w ogóle się nie wykonuje.
function hasPendingRuns(ctx) {
  return db.getQueuedRuns().some((run) => !ctx.skipRunIds.has(run.id));
}
...
async function processQueue() {
  ...
  try {
    while (true) {
      // Karencja po wybudzeniu (R11) obejmuje WSZYSTKIE joby, nie tylko run_on_wake:
      // job Pulsa to w ogromnej większości spawn CLI `claude`, który i tak gada z API po
      // sieci, a ENOTFOUND nie pyta o flagę. Filtr per-job kupowałby dwie klasy w pickerze
      // za cenę zgadywania, który job „wymaga sieci" — koszt globalnej karencji to jedno
      // opóźnienie rzędu pół minuty, raz na wybudzenie.
      const now = Date.now();
      // Detekcja wybudzenia TU, a nie tylko w callbacku heartbeatu: po pobudce libuv
      // odpala zaległe timery w kolejności due-time, a cron joba `* * * * *` (inbox sync)
      // typowo wypada wcześniej niż kolejne tyknięcie — bez tego pierwszy run po pobudce
      // startowałby BEZ karencji i padał na ENOTFOUND (R11). Koszt na normalnej ścieżce
      // to jedno odejmowanie: zero await, zero zapytania do DB (R1 nienaruszone).
      noteWakeIfGap(now);
      const deferring = shouldDeferAfterWake(wakeDetectedAt, now);
      // Ścieżka bez wybudzenia jest DOKŁADNIE dzisiejsza: zero await, zero dodatkowego
      // zapytania do DB — krótki run nie może zwolnić o ani jeden tick (R1).
      if (!deferring) startEligibleRuns(ctx);

      // Czekamy na koniec karencji tylko wtedy, gdy jest po co: przy pustej kolejce pętla
      // ma się domknąć, a nie trzymać `queueProcessing` przez pół minuty.
      const waitingForGrace = deferring && hasPendingRuns(ctx);
      ...
      if (ctx.inFlight.size === 0 && !waitingForGrace) break;

      const running = [...ctx.inFlight.values()].map((entry) => entry.promise);
      // Timer karencji jest WŁASNYM bodźcem pętli: run zakolejkowany w oknie karencji
      // startuje po jej upływie także wtedy, gdy nikt już nie zadzwoni dzwonkiem.
      const graceTimer = waitingForGrace ? delayPromise(wakeGraceRemainingMs(wakeDetectedAt, now)) : null;
      try {
        await Promise.race([...running, waitForNewWork(), ...(graceTimer ? [graceTimer.promise] : [])]);
      } finally {
        // Zwolnienie idempotentne — pętla może wyjść tędy także przez wyjątek.
        if (graceTimer) graceTimer.cancel();
      }
    }
  } finally {
    stopListening();
...
// Odcisk TEGO procesu w state — wykrywanie DRUGIEGO żywego schedulera na tej samej bazie.
// Incydent 07.08: zabłąkany `server.js` (leftover po testach) żył 26 h z własnym schedulerem;
// bez portu jest niewykrywalny z zewnątrz, a gdyby wskazywał produkcyjną bazę, dublowałby
// joby (inbox sync co minutę × 2 = ta sama klasa awarii co konflikt ról). Mechanizm: każdy
// proces pisze swój odcisk przy heartbeacie; jeśli przy tyknięciu w state siedzi CUDZY odcisk,
// to ktoś inny nadpisał go między naszymi tyknięciami — czyli żyje i tyka w tę samą bazę.
// Świadomie NIE fail-fast: po legalnym restarcie (updater: kill → start w 2 s) w state
// zostaje odcisk martwego poprzednika i wyjście na jego widok zabiłoby każdy restart.
// Martwy proces nie pisze — więc dopiero PONOWNE pojawienie się cudzego odcisku po naszym
// zapisie dowodzi żywego intruza.
const INSTANCE_ID = `${process.pid}:${Date.now()}`;
const INSTANCE_STATE_KEY = 'scheduler_instance';

// Czysta decyzja (testowalna bez timerów): czy odcisk zastany w state dowodzi obcego
// ŻYWEGO schedulera. Obcy = inny niż nasz; żywy = pojawił się PO naszym ostatnim zapisie
// (my pisaliśmy ostatni, więc każda zmiana wartości to cudza ręka).
function isForeignSchedulerAlive({ stored, mine }) {
  return typeof stored === 'string' && stored !== '' && stored !== mine;
}

function noteForeignScheduler(stored) {
  console.error(
    `[scheduler] 🔴 DRUGI scheduler pisze do tej samej bazy (odcisk "${stored}", mój "${INSTANCE_ID}") — ` +
      'dwa procesy server.js dublują joby! Znajdź i ubij zabłąkany proces: ' +
      '`ps aux | grep server.js` (macOS/Linux) albo Get-CimInstance Win32_Process (Windows).'
  );
}
...
function startHeartbeat() {
  ...
  // Pierwszy zapis odcisku BEZ sprawdzania zastanej wartości — to może być ślad legalnie
  // ubitego poprzednika (restart, aktualizacja). Wykrywamy dopiero od drugiego tyknięcia.
  db.setState(INSTANCE_STATE_KEY, INSTANCE_ID);
  heartbeatInterval = setInterval(() => {
    const now = Date.now();
    // Luka wall-clock MIĘDZY TYKNIĘCIAMI to jedyny wiarygodny ślad snu przy żyjącym
    // procesie. Znacznik z DB go nie pokaże: po pobudce Node odpala zaległy callback
    // natychmiast i nadpisuje last_active_at świeżą wartością — luka zniknęłaby, zanim
    // ktokolwiek zdąży ją przeczytać (dokładnie ten sam mechanizm, na którym stoi
    // startSleepAwareTimeout w executorze).
    noteWakeIfGap(now);
    lastHeartbeatAt = now;
    db.setState('last_active_at', new Date(now).toISOString());
    const stored = db.getState(INSTANCE_STATE_KEY);
    if (isForeignSchedulerAlive({ stored, mine: INSTANCE_ID })) noteForeignScheduler(stored);
    db.setState(INSTANCE_STATE_KEY, INSTANCE_ID);
...
function stopHeartbeat() {
  ...
  // Po ponownym starcie luka „od ostatniego tyknięcia sprzed stopu" nie jest snem maszyny.
  lastHeartbeatAt = null;
}

// Restart daemona po długiej przerwie to też powrót do życia: po reboocie maszyny (Mac, VPS)
// sieć wstaje razem z nami, więc pierwszy run musi dostać tę samą karencję co po wybudzeniu.
// MUSI być wołane PRZED startHeartbeat — heartbeat nadpisuje last_active_at przy pierwszym
// wywołaniu, więc po nim ślad downtime'u już nie istnieje.
function detectWakeFromDowntime(now = Date.now()) {
  const lastActive = db.getState('last_active_at');
  if (!lastActive) return;

  const lastMs = Date.parse(lastActive);
  // Ten sam próg co heartbeat: `last_active_at` jest zapisywane właśnie tyknięciem
  // heartbeatu, więc zwykły restart serwisu zostawia lukę „ile zostało do tyknięcia
  // + czas startu" — przy progu równym okresowi każdy redeploy udawałby wybudzenie.
  if (!Number.isFinite(lastMs) || !isWakeGap(lastMs, now, WAKE_GAP_MS)) return;

  markWakeDetected(now);
  console.log(`[scheduler] start po ${Math.round((now - lastMs) / 1000)} s przerwy — karencja ${WAKE_GRACE_MS / 1000} s przed startem runów`);
}
...
function start() {
  // Karencja PRZED detekcją przegapionych: to właśnie te joby (run_on_wake) lądują
  // w kolejce jako pierwsze po pobudce i to one padały na ENOTFOUND (R11).
...
module.exports = {
  ...
  processQueue,
  computeMissedJobs,
  isForeignSchedulerAlive,
  classifyJob,
  shouldDeferAfterWake,
  wakeGraceRemainingMs,
  isWakeGap,
  markWakeDetected,
  getWakeDetectedAt,
  WAKE_GRACE_MS,
