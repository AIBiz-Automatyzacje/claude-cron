...
// duplikujemy go) POSZERZONY o nasze runy w locie. Executor bywa o włos do przodu:
// wpis znika przy domknięciu procesu, a retry-check leci dopiero po nim, więc sam
// executor na moment pokazałby wolny slot dla runu, który jeszcze nie jest rozliczony.
function mergeActiveRuns(executorRuns, inFlight) {
  ...
  for (const [runId, entry] of inFlight) byRunId.set(runId, { runId, jobId: entry.jobId });
  return [...byRunId.values()];
}

function startRun(run, inFlight) {
  const promise = (async () => {
    try {
...
function startEligibleRuns(inFlight) {
  ...
  for (const run of picked) startRun(run, inFlight);
}
...
async function processQueue() {
  ...
  const inFlight = new Map(); // runId → { jobId, promise } — promise żyje tyle, co run + retry-check

  try {
    while (true) {
      startEligibleRuns(inFlight);
      ...
      const running = [...inFlight.values()].map((entry) => entry.promise);
