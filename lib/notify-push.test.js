...
test('pushNotifySettings: vpsUrl bez protokołu → invalid_vps_url bez rzucania, zero wywołań sieciowych', async () => {
  // Arrange — 'localhost:7777' bez http(s):// rzucałby TypeError: Invalid URL z new URL()
  const { fetchImpl, calls } = makeFetchMock([]);

  // Act — kontrakt: NIGDY nie rzuca
  const result = await pushNotifySettings({ vpsUrl: 'localhost:7777', settings: SETTINGS, fetchImpl });

  // Assert
  assert.deepEqual(result, { ok: false, reason: 'invalid_vps_url' });
