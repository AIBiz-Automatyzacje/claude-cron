...
// Seam plain-text dla ask (asystent głosowy): surowy tekst bez embedów i BEZ extractResult
// — ask używa `--output-format text`, nie stream-json, więc parsowanie wpisu result nie ma
// czego szukać. Konfiguracja rozwiązywana w czasie wysyłki (state > env), jak sendNotification.
async function sendPlain(text) {
  const { discordWebhookUrl } = resolveNotifyConfig(db.getState, process.env);
  if (!discordWebhookUrl) return;

  for (const chunk of smartSplit(text, DISCORD_MAX_LEN)) {
    await postWebhook(discordWebhookUrl, { content: chunk });
  }
}

module.exports = { sendNotification, sendFailureNotification, sendPlain };
