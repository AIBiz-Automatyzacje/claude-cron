...
function resolvePortableNodeBin(execPath, repoDir) {
  ...
  // Kotwiczymy w TYM repo, nie na samym segmencie `.node`: katalog instalacji jest wyborem
  // usera, więc dwie instancje obok siebie to scenariusz wspierany. Gołe `includes('.node')`
  // sprawiało, że setup instancji B uruchomiony portable Nodem instancji A wypalał w pliście
  // B ścieżkę do binarki A — po skasowaniu A agent B nie wstawał, a launchd raportował
  // wyłącznie kod błędu.
  if (execPath && execPath.startsWith(`${nodeBase}${path.sep}`)) return execPath;
...
// Wstrzykiwalne I/O (wzorzec `db.setDbPath` / `claude-spawn.setClaudeBin`): produkcja bierze
// domyślny obiekt, testy atrapę. Bez tego KOLEJNOŚCI kroków installMac() (unload przed zapisem
// plista i przed `load`, katalog logów przed `load`) nie da się zweryfikować inaczej niż realnym
// `launchctl load` w sesji GUI — a to jest dokładnie ta klasa błędu, przed którą ostrzega
// learned-pattern 2026-07-28 (testy czystych funkcji przechodzą przy złamanym zachowaniu systemu).
const REAL_IO = {
  mkdirp: dir => fs.mkdirSync(dir, { recursive: true }),
  writeFile: (file, content) => fs.writeFileSync(file, content, 'utf-8'),
  exists: file => fs.existsSync(file),
  unlink: file => fs.unlinkSync(file),
  launchctl: (args, options) => execFileSync('launchctl', args, options),
  log: message => console.log(message),
  warn: message => console.warn(message),
};

function installMac(io = REAL_IO) {
  io.mkdirp(path.dirname(PLIST_PATH));
  ...
  io.mkdirp(path.dirname(macLogFile()));
  ...
  unloadAgent(PLIST_PATH, io);
  removeLegacyAgents(io);
  io.writeFile(PLIST_PATH, generatePlist());
  io.launchctl(['load', '-w', PLIST_PATH], { stdio: 'inherit' });
  return PLIST_PATH;
}
...
// Reset flagi „ostrzegaliśmy już" — WYŁĄCZNIE dla testów. Bez tego stan przecieka między
// przypadkami: test na ostrzeżenie przechodził tylko dopóki był PIERWSZYM, który wywoła pad
// `launchctl`, a dołożenie wcześniejszego dawało zero ostrzeżeń i fałszywą czerwień.
function resetLaunchctlWarning() {
  launchctlWarned = false;
}

function readLaunchctlList(io = REAL_IO) {
  try {
    return io.launchctl(['list'], { encoding: 'utf-8' });
  } catch (err) {
    ...
    if (!launchctlWarned) {
      ...
      io.warn(`[platform] launchctl list nie odpowiedział: ${err.message}`);
    }
    return '';
  }
}
...
function unloadAgent(plistPath, io = REAL_IO) {
  try {
    io.launchctl(['unload', plistPath], { stdio: 'ignore' });
...
function removeLegacyAgents(io = REAL_IO) {
  for (const label of LEGACY_PLIST_LABELS) {
    const legacyPath = legacyPlistPath(label);
    if (!io.exists(legacyPath)) continue;
    const unloaded = unloadAgent(legacyPath, io);
    io.unlink(legacyPath);
    io.log(`[platform] Usunięto stary agent launchd: ${label}`);
    ...
    if (!unloaded.ok) {
      io.warn(`[platform] launchctl unload ${label} nie powiódł się (${unloaded.error.message}) — jeśli agent biegł, ubij go ręcznie albo zrestartuj Maca`);
    }
  }
}
...
module.exports = {
  ...
  // Funkcje z I/O — testowalne przez wstrzyknięcie atrapy `io` (patrz REAL_IO).
  removeLegacyAgents, unloadAgent, readLaunchctlList,
  resetLaunchctlWarning, // tylko dla testów — flaga ostrzeżenia żyje przez cały proces
  PLIST_PATH, PLIST_LABEL, LEGACY_PLIST_LABELS,
};
