...
test('boot-seed: rola agent WYŁĄCZA startery zaseedowane przed nadaniem roli (dziura czasowa instalatora)', () => {
  // Instalator VPS: pierwszy boot daemona (i seed) dzieje się PRZED onboardingiem,
  // który ustawia rolę — startery powstają jak dla clienta (incydent srv1362522 07.08).
  seedStarterJobs({ availableSkillNames: ALL_SKILLS });
  const own = db.createJob({ name: 'Mój własny job', skill_name: 'memory-update', cron_expr: '0 9 * * *' });
  const manualOff = db.getAllJobs().find((j) => j.name === 'Reflect tygodniowy');
  db.updateJob(manualOff.id, { enabled: 0 }); // ręczne wyłączenie — ma pozostać nietknięte, bez wpisu w disabled

  db.setState('inbox_role', 'agent');
  const result = seedStarterJobsAtBoot({ availableSkillNames: ALL_SKILLS });

  assert.equal(result.gated, 'agent');
  assert.equal(result.disabled.length, 4); // 5 starterów minus ręcznie wyłączony
  assert.ok(!result.disabled.includes('Reflect tygodniowy'));
  for (const job of db.getAllJobs()) {
    if (job.name === 'Mój własny job') continue;
    assert.equal(job.enabled, 0, `starter "${job.name}" ma być wyłączony na agencie`);
  }
  assert.equal(db.getJob(own.id).enabled, 1); // nie-starter nietknięty

  // Idempotencja: drugi boot niczego już nie wyłącza (zero „naprawiania" w kółko)
  const second = seedStarterJobsAtBoot({ availableSkillNames: ALL_SKILLS });
