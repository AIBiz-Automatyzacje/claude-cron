...
test('readWindowsPersistedEnv: spawn dostaje timeout i killSignal', () => {
  ...
  // Serwer działa bez konsoli — bez windowsHide każdy odczyt (co 30 s przy otwartym
  // panelu) mrugał userowi oknem PowerShella przez pół ekranu.
  assert.strictEqual(seenOptions.windowsHide, true, 'spawnSync musi ukrywać okno konsoli');
});
