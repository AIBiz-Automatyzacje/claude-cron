...
const { matchWebhookToken, matchAskToken, queryParams } = require('./webhook');
...
// --- queryParams: dane dla webhooków wyzwalanych GET-em ---

test('queryParams: wyciąga parametry z URL webhooka', () => {
  assert.deepEqual(
    queryParams('/webhook/abc123?id=recABC&kto=slawek'),
    { id: 'recABC', kto: 'slawek' },
  );
});

test('queryParams: brak query → pusty obiekt, nie null', () => {
  assert.deepEqual(queryParams('/webhook/abc123'), {});
  assert.deepEqual(queryParams('/webhook/abc123?'), {});
});

test('queryParams: dekoduje wartości zakodowane procentowo', () => {
  assert.deepEqual(
    queryParams('/webhook/abc?klient=Jan%20Kowalski&znak=%C5%81'),
    { klient: 'Jan Kowalski', znak: 'Ł' },
  );
});

test('queryParams: powtórzony klucz → ostatnia wartość', () => {
  assert.deepEqual(queryParams('/webhook/abc?id=pierwszy&id=drugi'), { id: 'drugi' });
});

test('queryParams: nie-string na wejściu → pusty obiekt', () => {
  assert.deepEqual(queryParams(undefined), {});
  assert.deepEqual(queryParams(null), {});
});
