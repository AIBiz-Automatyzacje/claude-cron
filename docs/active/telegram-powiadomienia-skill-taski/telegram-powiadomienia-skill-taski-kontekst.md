...
20. **Dostępność skilla przy seedzie liczona po `name` LUB `dir_name`** — lustrzane wobec `lib/skills.getSkill()`, bo executor woła prompt `/${skill_name}` (slash rozwiązuje się po obu). Seed idempotentny po nazwie joba, bez sentinela w state: pytanie `[T/n]` jest opt-in, więc re-run z „T" świadomie przywraca usunięte taski. (Faza 4, IU-8)
21. **Pad instalacji skilla `puls` nie przerywa setupu** — `installPulsSkill` łapie błąd kopiowania i wypisuje warn z instrukcją ręcznego skopiowania (skill to warstwa wygody, nie rdzeń instalacji); seed tasków odpalany PO smoke-teście DB, poza blokiem try (wzorzec `persistNotifySettings`). (Faza 4, IU-8/IU-9)
...
- **Review fazy 3 (2026-07-03)** — raport: `review-faza-3.md`. Gate: ⚠️ ZASTRZEŻENIA (0×P1, 2×P2, 17×P3, 2×OPERATOR; 31 surowych findings zdeduplikowane do 21). P2: (1) scenariusze [E2E] Unit 5 niewykonane — cały frontend modala (138 linii) bez weryfikacji zachowania, wykonalne headless na `localhost:7777`; (2) scenariusz z planu Unit 7 „harness bez pytania o Discord" niepokryty — `collect_config` mockowany w harnessie, regresja wiszącego `read` pod `curl|bash` przeszłaby suite. Kluczowe wnioski P3: „Pominięto Telegram" kłamie (sam token i tak idzie do state i na VPS), zero walidacji formatu wartości powiadomień na WSZYSTKICH granicach (regresja po usunięciu `is_valid_discord_webhook`), re-run instalatora cicho kasuje `DISCORD_WEBHOOK_URL` ze starego unitu systemd, kontrakt kluczy snake_case żyje w trzech miejscach. Bookkeeping: CLI odznaczone (setup.test.mjs 46/46, install-vps.test.sh 101/101, grep DISCORD → 0); E2E 0/0/3 skipped; 2 checkboxy operatora → „Operator checklist faza 3" (push na żywy VPS, pełny setup z prawdziwym botem).

- **Fix po review fazy 3 (2026-07-03, commit `1b65d21`)** — oba P2 domknięte: (1) 3/3 scenariusze [E2E] Unit 5 PASS przez agent-browser na izolowanej instancji (port 7799, świeża DB, czysty env — produkcyjny 7777 celowo nietknięty); (2) nowy Test 70 w `install-vps.test.sh` wykonuje REALNY `collect_config` ze stubem `ask_tty` (dokładnie 4/2 pytania, zero `discord|webhook`), suite 102/102 PASS. Pozostałe P3 (m.in. „Pominięto Telegram" a token idzie do state; brak walidacji formatu wartości) udokumentowane w `review-faza-3.md` i checkboxach „Do poprawy", nie blokują.
- **Faza 4 — ZAIMPLEMENTOWANA (2026-07-03), czeka na review.** Unit 8 (IU-8): `templates/starter-jobs.json` (4 szablony: Daily/Weekly memory update, Reflect tygodniowy, Poszukiwanie nowych skillów; `enabled=1`, `run_on_wake=1`, `discord_notify=0`, `telegram_notify=0`), `lib/starter-jobs.js(+test)` — czysta `computeStarterJobsToSeed(defs, existingJobs, availableSkillNames)` → `{toSeed, skipped:[{name, reason}]}` (enum `SKIP_REASON`: `exists`/`missing_skill`) + skorupa `seedStarterJobs()` z DI; `setup.mjs` — pytanie zbiorcze `[T/n]` po autostarcie, seed PO smoke-teście DB (`seedStarterJobsWithReport` z raportem dodanych/pominiętych). Unit 9 (IU-9): `skills/puls/SKILL.md` (frontmatter `name: puls` + description z triggerami PL + `allowed-tools: ["Bash","Read"]`; tabela endpointów REST, whitelist pól joba, czytanie stream-json, przykłady curl), `setup.mjs` — `copySkillDir` (czysty helper, `fs.cpSync` recursive+force — kopiowanie zamiast symlinku, re-run nadpisuje) + `installPulsSkill` do `~/.claude/skills/puls`. Walidacja: `npm test` 264/264 PASS (było 255; +9), `node --test lib/starter-jobs.test.js` 6/6, `node --test setup.test.mjs` 49/49; frontmatter SKILL.md sparsowany przez `gray-matter` (name/description/allowed-tools OK), `templates/starter-jobs.json` poprawny JSON; zero nowych zależności.
