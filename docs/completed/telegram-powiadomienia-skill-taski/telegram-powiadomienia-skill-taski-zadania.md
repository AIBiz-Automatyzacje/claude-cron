...
- [x] Weryfikacja: `grep -q TELEGRAM_BOT_TOKEN README.md CLAUDE.md` przechodzi; `npm test` zielony (regresja całości) (review fazy 5: grep exit 0; `npm test` 267/267 PASS, exit 0)

## Do poprawy po review fazy 5

Zero P1/P2 — gate CZYSTE. Poniższe P3 opcjonalne (szczegóły i rekomendacje w `review-faza-5.md`; findingi #1/#3/#6 raportu to trzy kąty tego samego problemu — połączone w pierwszy checkbox):

- [ ] 🟡 [P3] **.gitignore:6 + .claude/templates/e2e-env/.env.e2e.example:26** — deliverable Unitu 10 (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` w szablonie e2e-env) jest gitignorowany wzorcem `.env.*` i nietrackowany — istnieje tylko lokalnie, świeży clone nie dostanie szablonu, a README szablonu każe kopiować z pliku nieobecnego w repo. Fix: negacja `!.claude/templates/e2e-env/.env.e2e.example` w `.gitignore` + commit pliku (same placeholdery, zweryfikowane — zero sekretów; przed commitem zweryfikować zawartość ponownie) + udokumentować zmienne TELEGRAM_* (z notą o dedykowanym bocie testowym) w trackowanym `.claude/templates/e2e-env/README.md`
- [ ] 🟡 [P3] **README.md:334 + CLAUDE.md:45** — overclaim bezpieczeństwa: „nigdy nie pokazują pełnych wartości — tylko maskę" / „API zwraca wyłącznie maski" niezgodne z kodem — `GET /api/settings/notifications` zwraca `telegram.chat_id` w pełnej formie (`lib/notify-config.js:49`). Fix dokumentacyjny: „token i webhook maskowane (ostatnie 4 znaki); chat ID widoczny jawnie" — albo maskowanie chat_id w API (zmiana kodu poza zakresem fazy)
- [ ] 🟡 [P3] **README.md:135** — PRE-EXISTING: publiczny README używa jako przykładu realnego Tailscale IP VPS-a usera (`100.86.100.113`); ekspozycja minimalna (CGNAT nieosiągalny spoza tailnetu), ale przykład powinien być jawnie fikcyjny (np. `100.100.100.100`) — do poprawki przy najbliższej edycji README
- [ ] 🟡 [P3] **CLAUDE.md:44** — bullet „Powiadomienia" ~1100 znaków opisuje naraz 5 modułów — łamie konwencję „jeden moduł = jeden bullet" sekcji architektury (treść merytorycznie w 100% zgodna z kodem). Sugestia: rozbić na pod-bullety per moduł albo zostawić 2-3 zdania kluczowych inwariantów
